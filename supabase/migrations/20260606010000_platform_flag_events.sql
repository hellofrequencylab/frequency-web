...
-- (ADR-098 addendum). Append-only: who flipped which flag, when, old → new. The
...
comment on table public.platform_flag_events is
  'Append-only audit of platform_flags toggles (who/when/old->new). Operator-only via service role; surfaced at /admin/ai. See ADR-098.';
