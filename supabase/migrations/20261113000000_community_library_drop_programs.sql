-- Drop the Programs arm from the community_library RPC (Programs feature removal).
--
-- The Library "Programs" content type (ADR-109 outreach toolkits) is retired: the third
-- Library content type beside Practices and Journeys is gone. This migration rewrites the
-- ranked catalog RPC WITHOUT the `union all select 'program' … from programs` arm, so the
-- catalog now unions only Practices and Journeys.
--
-- Everything else is PRESERVED verbatim: the exact function signature
-- community_library(_type text, _pillar text, _limit int), the full RETURNS TABLE column
-- list including the card-times columns (duration_min / cadence / unit_count / unit_label),
-- the SEASON RANK enum values ghost/initiate/adept/master used for the endorser weight, the
-- scoring formula, and the REVOKE/GRANT block.
--
-- Postgres cannot change a RETURNS TABLE signature via CREATE OR REPLACE, so the function is
-- DROPped and re-created, then the REVOKE/GRANT is re-issued. The `programs` /
-- program_adoptions TABLES are intentionally NOT dropped here — that is a separate, clearly
-- labeled destructive migration (20261114000000_drop_programs_tables.sql).

drop function if exists public.community_library(text, text, int);

create function public.community_library(
  _type   text default null,   -- 'practice' | 'journey' | null (all)
