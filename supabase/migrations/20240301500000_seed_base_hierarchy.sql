-- Seed the foundational San Diego geography hierarchy + base demo circles.
--
-- REPLAY FIX: These rows (region/outpost/nexus/hub/circle IDs 11.. → 55..) were
-- historically applied to production via supabase/seed_hierarchy.sql, which is a
-- manual seed file and is NOT part of the migration ledger. Several later
-- migrations depend on them existing:
--   * 20240302000000_seed_circle_coords.sql  (UPDATEs circles 55..001/002/004)
--   * 20240303000000_seed_demo_circles.sql   (parent_id 11..002, hub_id 44..001/002,
--                                             circles 55..001-004)
--   * 20260603000002 / 20260603000003 demo migrations (geography ancestors)
-- On a fresh apply (db-tests) those parents were absent, so 20240303 failed with
-- a foreign-key violation on nexus_regions_parent_id_fkey. This migration makes
-- the ledger self-contained by seeding the same intended data the prod seed file
-- created. Fully idempotent (ON CONFLICT (id) DO NOTHING), so it is a no-op on any
-- DB where seed_hierarchy.sql already ran.
--
-- Must run AFTER 20240201000000 (adds circles.neighborhood/latitude/longitude and
-- seeds topical_channels) and BEFORE 20240302000000 (first consumer).
