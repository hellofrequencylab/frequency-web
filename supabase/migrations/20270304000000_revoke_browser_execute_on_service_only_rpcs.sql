-- 🔴 DRAFT — AUTHORED, NOT APPLIED. Do not merge this file on its own.
--
-- `pnpm check:migrations` rule 4 (ADR-1007) compares this directory against
-- `supabase_migrations.schema_migrations` and fails on a repo file with no ledger row. Applying is
-- TWO steps, and the second is the one everybody forgets — see supabase/migrations/README.md.
-- Apply, then repair the ledger to carry version 20270304000000, then merge.
--
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
