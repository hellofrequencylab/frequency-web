-- =============================================================================
-- Seed the 4 OFFICIAL seasonal Journeys — Summer · "The Signal" (JOURNEYS.md §2/§4)
--
-- The canonical Season-1 official content drop (Build Phase P4): ONE official
-- journey_plans row per Domain (Mind · Body · Spirit · Expression), nested under the
-- active Seasonal Quest, each with 4 journey_plan_items on a sensible cadence mix
-- (2× Daily · 1× A few times a week · 1× Weekly), default_tier='current', and all
-- three intensity tiers (spark/current/deep) authored on each referenced practice.
--
-- Practices are referenced by TITLE against the SYSTEM library (created_by IS NULL)
-- seeded by the practices migrations — these 16 titles are the real ones in that
-- library, so every Journey is fully wired on a fresh DB. (Verified against prod;
-- ADR-198/199/200.) IDEMPOTENT: stable slugs + ON CONFLICT everywhere; safe to re-run.
-- Runs after the tier/completion schema migrations (…101000–102000), so the columns
-- + practice_tiers table already exist.
-- =============================================================================

BEGIN;

-- 1. Intensity tiers (Spark / Current / Deep) for every referenced practice. -----
...
FROM (VALUES
  ...
  ('Morning movement','spark','A few easy reps','Five minutes of easy movement — roll the joints, a few squats, shake out the night. Just enough to wake the body.',5),
  ('Morning movement','current','Morning movement','Fifteen minutes getting the body going — a flow, a short routine, a brisk start to the day.',15),
  ('Morning movement','deep','A full morning routine','Thirty minutes of deliberate movement to open the body and set the tone for the whole day.',30),
  ...
  -- Spirit · Tune In
  ('Daily meditation','spark','Five before screens','Five minutes of stillness, eyes closed, before the inputs start. No agenda, just the breath.',5),
  ('Daily meditation','current','Daily meditation','Fifteen minutes following the breath, same seat, same time. Returning when the mind wanders is the practice.',15),
  ('Daily meditation','deep','An extended sit','Thirty minutes of sitting practice, breath work into the silence, before any device is touched.',30),
  ('Gratitude journaling','spark','One good thing','Name one specific good thing from the last day. Thirty seconds is plenty.',2),
  ('Gratitude journaling','current','Three specifics','Three concrete things you are grateful for — the actual moments, not the categories. Same notebook, same time.',10),
  ('Gratitude journaling','deep','A gratitude letter','Write at length to someone or something you appreciate, tracing exactly why it matters. Send it if you can.',20),
  ('Breathwork','spark','A few rounds','Inhale four, exhale six, for two minutes. The fastest lever you have on a wound-up system.',3),
  ('Breathwork','current','Breathwork reset','Ten minutes of down-regulating breath — long, slow exhales — somewhere you will not be interrupted.',12),
  ('Breathwork','deep','A full breath journey','Twenty-plus minutes of structured breathwork, ideally guided. Set time and space; it goes deep.',25),
  ('Evening reflection','spark','One line on the day','A single sentence on how the day actually went. Close the loop before sleep.',3),
  ('Evening reflection','current','Evening reflection','Ten quiet minutes looking back on the day — what landed, what you would change, what you are letting go.',10),
  ('Evening reflection','deep','A weekly review','A longer reflection across the week: the patterns, the wins, the one thing to carry forward.',25),
  -- Expression · Broadcast
  ('One photo a day','spark','One frame','Take a single photograph of something that catches your eye. Two minutes, one image.',2),
  ('One photo a day','current','One photo a day','Make one considered photograph — frame it, notice the light, choose the moment. Keep the creative eye open.',10),
  ('One photo a day','deep','A photo walk','A deliberate half hour with the camera, hunting for a handful of frames you would actually print.',30),
  ...
  ('Dance one song','current','Dance one song','A full song of free movement — joy in a single hit. Let it shake loose whatever you have been sitting in.',4),
...
-- 2. The 4 official Journeys + their 4 items each. ------------------------------
DO $$
DECLARE
  v_season int; v_suffix text; v_quest_id uuid; v_domain_id uuid; v_plan_id uuid; j record; it record;
BEGIN
  SELECT season_number INTO v_season FROM public.seasons WHERE status = 'active' LIMIT 1;
  IF v_season IS NULL THEN SELECT max(season_number) INTO v_season FROM public.seasons; END IF;
  ...
  IF v_quest_id IS NULL THEN SELECT id INTO v_quest_id FROM public.quests WHERE status = 'active' ORDER BY sort_order ASC LIMIT 1; END IF;
  IF v_quest_id IS NULL THEN RAISE NOTICE 'No active Seasonal Quest — skipping official Journey seed.'; RETURN; END IF;
  ...
  LOOP
    ...
    INSERT INTO public.journey_plans
      (slug, title, summary, intro, emoji, accent, visibility, official, quest_id, status, season_locked, target_weeks, min_practices_per_day, completion_gems, published_at)
    VALUES
      ('official-' || v_suffix || '-' || j.domain_slug, j.title, j.summary, j.intro, j.emoji, j.accent, 'public', true, v_quest_id, 'approved', true, 8, 1, 30, now())
    ON CONFLICT (slug) DO NOTHING;

    SELECT id INTO v_plan_id FROM public.journey_plans WHERE slug = 'official-' || v_suffix || '-' || j.domain_slug;
    CONTINUE WHEN v_plan_id IS NULL;

    UPDATE public.journey_plans SET
      official = true, quest_id = v_quest_id, status = 'approved', visibility = 'public',
      season_locked = true, target_weeks = 8, min_practices_per_day = 1, completion_gems = 30,
    ...
    FOR it IN
      ...
      FROM (VALUES
        ('mind','Deep work block','Daily',1),('mind','Digital sunset','Daily',2),('mind','Read ten pages','A few times a week',3),('mind','Plan tomorrow tonight','Weekly',4),
        ('body','Daily walk','Daily',1),('body','Morning movement','Daily',2),('body','Strength session','A few times a week',3),('body','Time in nature','Weekly',4),
        ('spirit','Daily meditation','Daily',1),('spirit','Gratitude journaling','Daily',2),('spirit','Breathwork','A few times a week',3),('spirit','Evening reflection','Weekly',4),
        ('expression','One photo a day','Daily',1),('expression','Voice journal','Daily',2),('expression','Make music','A few times a week',3),('expression','Dance one song','Weekly',4)
      ) AS i(domain_slug, p_title, cadence, sort_order)
      WHERE i.domain_slug = j.domain_slug ORDER BY i.sort_order
    LOOP
      INSERT INTO public.journey_plan_items (plan_id, practice_id, domain_id, cadence, default_tier, sort_order, note)
      SELECT v_plan_id, p.id, v_domain_id, it.cadence, 'current', it.sort_order, NULL
      FROM public.practices p WHERE p.title = it.p_title AND p.created_by IS NULL ORDER BY p.created_at LIMIT 1
      ON CONFLICT (plan_id, practice_id) DO UPDATE
        SET cadence = excluded.cadence, sort_order = excluded.sort_order, default_tier = excluded.default_tier, domain_id = excluded.domain_id;
    END LOOP;
  END LOOP;
END $$;
