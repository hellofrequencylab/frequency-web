...
describe('agent packets (ADR-1412)', () => {
  ...
  it('CLI --json names no closed LIVE-412 or SCAN-644 on the scan lane', () => {
    ...
    expect(body.packets.some((p: { id: string }) => p.id === 'SCAN-644')).toBe(false)
    if (body.next[0]) {
      expect(body.next[0].derivedLane).toBe('scan')
      expect(body.next[0].id).not.toBe('LIVE-412')
    }
  })
