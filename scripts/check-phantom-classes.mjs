...
const DECLARED = [
  ...
  // ADR-949 — the fluid page title, the role that retires a responsive RAMP rather than a
  // literal. Asserted here for the reason the list exists: it is a `clamp()` under a name
  // Tailwind has no default for, so an unbridged declaration emits nothing and reads fine.
  'text-page-title-lg',
