...
// The bundle did not land. Every one of those files was still the 2026-08-03 export, and
// `on-media.card.html` never arrived at all. So:
//
//   • `lib/theme/dawn-divergence.test.ts` read `tokens/colors.css` and was GREEN, because the
...
//     asked DAWN to apply five corrections DAWN APPLIED THREE WEEKS AGO. That is the stale-row
//     failure the divergence test's own header describes (`--color-text-on-primary`, eleven days),
//     recurring one level up: not a stale row in the sheet, a stale FILE under the whole sheet.
//
// 🔴 AND THEN THE BUNDLE STOPPED BEING ONE ROUND AT ALL (2026-08-26). `tokens/colors.css` was
// brought current by TRANSCRIPTION rather than export: its values were written from what CHANGES.md
// §2 documents, with the twelve additions taking their hexes from `PROD-AHEAD.md` §5, because
// CHANGES.md names those twelve but prints no values. Every other file is still the 2026-08-03
// photocopy, and deliberately so — the rest of the round is DESIGN work (four hero rungs, a
// rewritten nav rail, new banner copy, a whole new guideline card), and writing those from a prose
// description would be inventing an artifact rather than copying one. DAWN itself made exactly that
// mistake this round: it drew a hero plate from a description that production had deleted on an
// owner ruling (PROD-AHEAD §6a).
//
// So a single round label became false in BOTH directions — '2026-08-03' rejects the one file that
// IS current, '2026-08-25' demands five that are not — and provenance moved onto each marker, where
// it can actually be true. This guard fails in every direction that matters:
//
//   1. A marker declared `pending` APPEARS → someone re-exported or transcribed and did not say so.
//      This is the dangerous direction: a refreshed `colors.css` closes declared divergences, and a
//      sheet that still lists them would ask DAWN to re-apply what it already has — or worse, be
//      "reconciled" by putting DAWN's old values back into `app/globals.css` and un-fixing three AA
//      failures production measured.
//   2. A marker declared `transcribed` or `exported` GOES MISSING → the file regressed, or someone
//      dropped an older copy over it.
//   3. A path CHANGES.md names is absent and undeclared, or a declared-absent path arrives.
//
// It never asserts "the bundle is up to date", because it is not, and a guard that cannot fire
// honestly gets routed around and then reads as coverage (ADR-970). The remaining gap is tracked as
// a backlog row with an owner action; this file's job is to keep the DECLARATION true.
...
/**
 * The bundle is a MIXTURE of rounds, so it carries no single label. Kept as an export because it
 * reads in failure text and in the sibling test; `bundleIsCurrent()` is derived, never declared.
 */
export const BUNDLE_ROUND = 'mixed'

/**
 * Strings the 2026-08-25 round says it wrote into named bundle files, each with its PROVENANCE:
 *
 *   `transcribed` — written here from CHANGES.md's own tables. The marker MUST be present.
 *   `exported`    — arrived as a real DAWN export. The marker MUST be present.
 *   `pending`     — still the 2026-08-03 photocopy. The marker MUST be ABSENT.
 *
 * Each marker is quoted from CHANGES.md's own change tables, so this is DAWN's claim about its own
 * export, not our summary of it, and a marker is chosen to be unmistakable: it appears only if that
 * round's change is present.
 */
...
/** True only when EVERY marker has arrived, by either route. Derived from the markers. */
export const bundleIsCurrent = () =>
  ROUND_MARKERS.every((m) => m.state === 'transcribed' || m.state === 'exported')

/** Markers still waiting on a real DAWN export. */
export const pendingMarkers = () => ROUND_MARKERS.filter((m) => m.state === 'pending')
 ...
 * existence rather than by content. It is NOT transcribable: a guideline card is design work.
 */
...
export function run() {
  ...
  if (round !== CHANGES_ROUND) {
    failures.push(
      `CHANGES.md is the ${round ?? 'unparseable'} round; this guard declares ${CHANGES_ROUND}.\n` +
        ...
        `    CHANGES.md, set CHANGES_ROUND, and set each marker's state.`,
    )
  ...
  for (const p of named.filter((p) => !absent.has(p) && !existsSync(join(BUNDLE, p)))) {
    failures.push(
  ...
  for (const p of DECLARED_ABSENT.filter((p) => existsSync(join(BUNDLE, p)))) {
    failures.push(
  ...
  // 3. Every marker must match its declared provenance, in whichever direction it is wrong.
  for (const { file, marker, state, proves } of ROUND_MARKERS) {
    ...
    const arrived = state === 'transcribed' || state === 'exported'

    if (arrived && !present) {
      failures.push(
        `\`${file}\` is declared ${state} but does NOT contain "${marker}".\n` +
          `    It must carry ${proves}. Either the file regressed, or an older copy was dropped\n` +
          `    over it, or the declaration is wrong.`,
      )
    }
    if (!arrived && present) {
      failures.push(
        `\`${file}\` NOW CONTAINS "${marker}" but is declared \`pending\`.\n` +
          `    🔴 THE DECLARATION AND THE LEDGER MUST MOVE WITH IT, IN THIS CHANGE:\n` +
          `      1. set this marker's state to 'exported' (or 'transcribed') here;\n` +
          `      2. re-derive the ledger in lib/theme/dawn-divergence.test.ts — a refreshed\n` +
          `         colors.css moves declared divergences and that test will say which;\n` +
          `      3. rewrite design_handoff/PROD-AHEAD.md so it stops describing a state that\n` +
          `         has changed.\n` +
          `    Do NOT reconcile by moving DAWN's values into app/globals.css: three of the five\n` +
          `    corrections are AA fixes production measured, and DAWN took production's value.`,
      )
    }
  }

  // 4. While anything is still pending, the sheet has to say so. PROD-AHEAD.md is the artifact the
  //    next outbound handoff copies verbatim; if it reads as current while resting on a mixed
  //    bundle, the next round re-sends applied work.
  if (!bundleIsCurrent() && !sheet.includes(CHANGES_ROUND)) {
    failures.push(
      `design_handoff/PROD-AHEAD.md does not mention the ${CHANGES_ROUND} round, and parts of the\n` +
        `    bundle are still the 2026-08-03 photocopy. The sheet must carry that state.`,
    )
  }

  const pending = pendingMarkers()
  if (pending.length) {
    notes.push(
      `bundle is MIXED: tokens/colors.css is transcribed to the ${CHANGES_ROUND} round; ` +
        `${pending.length} marker(s) still on the 2026-08-03 photocopy\n` +
        `    (${pending.map((m) => m.file).join(', ')}).\n` +
        `    That gap is DECLARED and tracked — the rest is design work and needs a real DAWN export.`,
    )
  }
...
if (invokedDirectly) {
  ...
  console.log(
    green(
      `✓ DAWN bundle contract: ${ROUND_MARKERS.length} round markers match their declared ` +
        `provenance, ${DECLARED_ABSENT.length} declared-absent path(s) still absent.`,
    ),
  )
}
