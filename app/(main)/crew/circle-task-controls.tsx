  ...
  return (
    <span className="flex flex-col items-end gap-1">
      <button
        onClick={handleClick}
        disabled={isPending}
        className="shrink-0 flex min-h-11 items-center gap-1 rounded-lg px-3 py-1 text-meta font-semibold bg-primary text-on-primary hover:bg-primary-hover transition-colors disabled:opacity-50 motion-reduce:transition-none"
      >
...
export function ReleaseTaskButton({ taskId }: { taskId: string }) {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={isPending}
      className="shrink-0 flex min-h-11 items-center gap-1 rounded-lg px-3 py-1 text-meta font-semibold bg-surface-elevated text-muted hover:bg-border-strong transition-colors disabled:opacity-50 motion-reduce:transition-none"
    >
