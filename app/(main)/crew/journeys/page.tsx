...
export default async function ArcsPage() {
  ...
  const [{ data: chains }, { data: allSteps }, { data: allProgress }] = await Promise.all([
    admin.from('journey_chains').select('*').order('sort_order'),
    admin.from('journey_steps').select('*').order('step_order'),
    admin.from('journey_progress').select('*').eq('profile_id', profile.id),
  ])
