...
export default async function NexusCrmPage({
  ...
  // The audience: active members of every circle in the nexus's subtree (display read capped;
  // the viewer filters client-side over what it holds).
  const members = await loadPlaceTreeCrmRoster({ kind: 'nexus', id: nexus.id })

  return (
    <DashboardTemplate
      eyebrow="Manage"
      title="Message Members"
      description={`Everyone across the hubs and circles of ${nexus.name} in one place. Pick a member to see their story, then send them a message.`}
    ...
    >
      <LeaderCrmViewer
        members={members}
        loadDetail={loadNexusCrmDetail.bind(null, nexus.slug)}
        openDm={openNexusMemberDm.bind(null, nexus.slug)}
        empty={{
