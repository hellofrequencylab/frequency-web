...
//
// The Guided section of a Journey's Inspector rail: the Spark's three dials (mood · directions ·
// lock) run over the LIVE Journey. Same four steps as every other entity (see
// lib/studio/kernel/redraw.ts): resolve the pins, DELETE them from the patch, diff what is left,
// write only what moved.
//
// THE JOURNEY IS THE ONE THAT GENUINELY DIFFERS, in two ways worth naming:
//
//   1. Vera's Journey editor returns OPERATIONS over a block tree, not a flat patch. A redraw
//      consumes exactly two of them: `identity` (the name, the promise, the overview) and
//      `phase` (a week's focus). `add_practice`, `practice`, and `remove` are IGNORED here on
//      purpose. "Draft it again" re-words a Journey; it does not silently restructure the
//      practices an author placed, and an author who wants that has the editor's own Vera box.
//   2. The weekly ARC is a repeated collection spread over `journey_plan_items` rows, so it
//      diffs as ONE path (`arc`, the manifest's own repeat) and writes as N row updates. That is
//      also what makes the `arc` pin real: the kernel resolves a repeat's section to its
//      collection path (ADR-996) and deletes it like any other field.

/** One week of the arc, as the redraw reads and writes it. */
...
/** The manifest paths a Journey redraw may touch. */
const JOURNEY_REDRAW_PATHS = ['title', 'summary', 'intro', 'arc'] as const

/** The arc as one readable block, so a week that moved shows up in the diff beside the plain
 *  fields instead of being invisible. Plain text, no dashes (voice canon). */
function arcDisplay(weeks: readonly JourneyWeek[]): string {
  return weeks
    .map((w, i) => `${i + 1}. ${w.title || 'Untitled week'}${w.focus ? ` (${w.focus})` : ''}`)
    .join('\n')
}

/** What one Journey redraw did: what moved, what the pins held, and the previous values of
 *  exactly the changed paths (feed straight back to restoreJourneyAction to undo). */
export interface JourneyRedrawResult {
  changes: FieldChange[]
  kept: string[]
  before: JourneyRedrawState
}

/** The subset of a Journey a redraw can put back. Only the paths that actually moved are set. */
export interface JourneyRedrawState {
  title?: string
  summary?: string
  intro?: string
  arc?: JourneyWeek[]
...
/** Load what the redraw reads: the plan's identity fields and its ordered Phase rows. */
async function journeyRedrawState(planId: string): Promise<{
  slug: string
  title: string
  summary: string
  intro: string
  weeks: JourneyWeek[]
  practiceLines: { phaseId: string; id: string; title: string; body: string }[]
} | null> {
  const admin = createAdminClient()
  const { data: planRow } = await admin
    .from('journey_plans')
    .select('id, slug, title, summary, intro')
    .eq('id', planId)
    .maybeSingle()
  const plan = planRow as { slug: string; title: string | null; summary: string | null; intro: string | null } | null
  if (!plan) return null

  const { data: itemRows } = await admin
    .from('journey_plan_items')
    .select('id, title, body, block_type, parent_id, sort_order')
    .eq('plan_id', planId)
  ...
  const weeks = items
    ...
    .map((b) => ({ id: b.id, title: b.title ?? '', focus: b.body ?? '' }))
  const phaseIds = new Set(weeks.map((w) => w.id))
  const practiceLines = items
    .filter((b) => b.parent_id !== null && phaseIds.has(b.parent_id) && b.block_type !== 'module')
    .map((b) => ({ phaseId: b.parent_id as string, id: b.id, title: b.title ?? '', body: b.body ?? '' }))
 ...
 *  keep, and draft it again. Owner-gated (assertOwner, the same gate the editor uses), and every
 *  pinned path is deleted from the patch before anything is compared or written. */
export async function redrawJourneyAction(
  planId: string,
  input: { mood?: string | null; directions?: string | null; locked?: readonly string[] },
): Promise<ActionResult<JourneyRedrawResult>> {
  const profileId = await assertOwner(planId)
  if (!profileId) return fail('Only the author can edit this journey.')
  const state = await journeyRedrawState(planId)
  if (!state) return fail('Journey not found.')

  const pins = declaredLockKeys(JOURNEY_MANIFEST, input.locked ?? [])
  await saveSteer('journey', planId, profileId, {
    mood: input.mood,
  ...
  const journey: JourneyForEdit = {
    ...
    intro: state.intro,
    phases: state.weeks.map((w) => ({
      id: w.id,
      title: w.title,
      focus: w.focus,
      practices: state.practiceLines
        .filter((p) => p.phaseId === w.id)
        .map((p) => ({ id: p.id, pillar: 'practice', title: p.title, body: p.body })),
    })),
  }

  const ops = await planJourneyEdits({
    request: redrawBrief({
      manifest: JOURNEY_MANIFEST,
      lead: 'Draft this Journey again, keeping every fact it already states true. Re-word the name, the promise, the overview, and each week focus. Do not add, remove, or re-word the practices inside the weeks.',
      mood: input.mood,
      directions: input.directions,
      locked: pins,
    }),
  ...
  // Fold the two ops a redraw consumes into a proposal in MANIFEST-PATH space. Every other op
  // kind is dropped here (see the header note), never applied.
  const proposal: JourneyRedrawState = {}
  const nextWeeks = state.weeks.map((w) => ({ ...w }))
  let arcMoved = false
  const weekById = new Map(nextWeeks.map((w) => [w.id, w]))
  for (const op of ops) {
    if (op.op === 'identity') {
      if (op.title !== undefined) proposal.title = op.title
      if (op.subtitle !== undefined) proposal.summary = op.subtitle
      if (op.intro !== undefined) proposal.intro = op.intro
    } else if (op.op === 'phase') {
      const week = weekById.get(op.id)
      if (!week) continue
      if (op.title !== undefined) week.title = op.title
      if (op.focus !== undefined) week.focus = op.focus
      arcMoved = true
    }
  }
  if (arcMoved) proposal.arc = nextWeeks

  // THE GUARANTEE: the pinned paths are deleted from the proposal here, before it is compared or
  // written. `identity` covers the name; `arc` is a repeated collection and is deleted the same way.
  const safe = applyLock(JOURNEY_MANIFEST, pins, proposal)
  ...
  const settled = settleRedraw({
    ...
    current: {
      ...
      intro: state.intro,
      arc: arcDisplay(state.weeks),
    },
    proposed: {
      ...(safe.title !== undefined ? { title: safe.title } : {}),
      ...(safe.summary !== undefined ? { summary: safe.summary } : {}),
      ...(safe.intro !== undefined ? { intro: safe.intro } : {}),
      ...(safe.arc ? { arc: arcDisplay(safe.arc) } : {}),
    },
  })
  const paths = settled.changedPaths.filter((p) => (JOURNEY_REDRAW_PATHS as readonly string[]).includes(p))
  if (paths.length === 0) {
    return fail('Vera kept this one as it is. Give her a direction, or unpin something, and try again.')
  }

  const write: JourneyRedrawState = {}
  const before: JourneyRedrawState = {}
  for (const path of paths) {
    if (path === 'title' && safe.title !== undefined) {
      write.title = safe.title
      before.title = state.title
    } else if (path === 'summary' && safe.summary !== undefined) {
      write.summary = safe.summary
      before.summary = state.summary
    } else if (path === 'intro' && safe.intro !== undefined) {
      write.intro = safe.intro
      before.intro = state.intro
    } else if (path === 'arc' && safe.arc) {
      write.arc = safe.arc
      before.arc = state.weeks
    }
  }
...
/** Put it back: write the pre-redraw values the diff handed the rail, through the same writer. */
export async function restoreJourneyAction(
  planId: string,
  before: JourneyRedrawState,
): Promise<ActionResult> {
  if (!(await assertOwner(planId))) return fail('Only the author can edit this journey.')
  const state = await journeyRedrawState(planId)
  if (!state) return fail('Journey not found.')
