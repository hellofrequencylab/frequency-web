...
export function ImportClient() {
  ...
  return (
    <div className="space-y-5">
      ...
      <details className="rounded-card border border-border bg-surface p-3 text-body-sm text-muted">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3">
          ...
          {text.trim().length > 0 && (
            <span className="text-meta text-subtle">{text.length.toLocaleString()} characters</span>
          )}
        </div>
...
function PreviewResult({
  ...
  return (
    <div className="space-y-4 border-t border-border pt-5">
      ...
      <div className="flex flex-wrap gap-2 text-meta text-muted">
      ...
      {aiSkipped && (
        <p className="rounded-card border border-border bg-surface-elevated/60 p-3 text-body-sm text-muted">
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-4">
      <div className="flex items-center gap-2">
        ...
        <span className="rounded-pill bg-surface-elevated px-2 py-0.5 text-meta font-medium capitalize text-muted">
          {item.category}
        </span>
        {item.confidence === 'low' && (
          <span className="text-meta text-subtle">⚠️ low confidence</span>
        )}
      ...
      {item.note && <p className="mt-2 text-body-sm text-muted">{item.note}</p>}
    </div>
  )
}
  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-2">
      ...
      {files.length === 0 ? (
        ...
        missing > 0 && <span className="text-meta text-subtle">+{missing} not selected</span>
      )}
    </div>
  )
}
...
function EventBody({ item }: { item: ClassifiedItem }) {
  ...
  return (
    <div className="mt-2 space-y-1">
      <p className="text-body font-bold text-text">{ev.title || 'Untitled event'}</p>
      {ev.description && <p className="text-body-sm text-text">{ev.description}</p>}
      <div className="mt-1 space-y-1 text-body-sm text-muted">
        {ev.startsAt && (
      ...
      <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-meta text-subtle">
...
function HousingBody({ item }: { item: ClassifiedItem }) {
  ...
  return (
    <div className="mt-2 space-y-1">
      <p className="text-body font-bold text-text">{h.title || 'Untitled listing'}</p>
      {h.description && <p className="text-body-sm text-text">{h.description}</p>}
      {bits.length > 0 && (
        <p className="mt-1 text-meta text-subtle">{bits.join(' · ')}</p>
      )}
      {h.contacts.length > 0 && (
        <p className="mt-1 flex items-center gap-1.5 text-meta text-subtle">
