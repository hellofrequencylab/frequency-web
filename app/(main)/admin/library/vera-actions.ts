...
const SYSTEM = withVoice(`You are Vera, drawing a new illustration "card" for Frequency's asset library (The Loom), in the exact house style.
...
- COLOR IS CLASSES ONLY — never a hex, rgb, or inline style. Put color on class="…" using ONLY these DAWN tokens:
...
export async function generateLoomCard(prompt: string): Promise<{ svg: string } | { error: string }> {
  ...
  try {
    const res = await completeText({
      system: SYSTEM,
      messages: [{ role: 'user', content: `Draw: ${clean}` }],
      tier: 'sonnet',
      maxTokens: 1600,
      cacheSystem: true,
    })
...
export async function saveLoomCard(input: {
  ...
  const { error } = await dbh.from('library_assets').insert({
    ...
    status: 'approved',
    visibility: 'public',
    config: { source: 'vera', prompt: (input.prompt || '').slice(0, 400), svg: checked.svg },
  })
