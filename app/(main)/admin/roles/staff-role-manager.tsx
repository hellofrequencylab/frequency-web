...
export function StaffRoleManager({ members }: { members: StaffMemberRow[] }) {
  ...
  return (
    <section className="rounded-2xl border border-border bg-surface lift-1">
      ...
      {error && (
        <p className="mx-4 mt-3 rounded-lg border border-danger bg-danger-bg/30 px-3 py-2 text-meta text-danger">{error}</p>
      )}
      ...
      {/* Current team */}
      {members.length === 0 ? (
        <p className="p-4 text-body-sm text-subtle">No team members yet. Add one above.</p>
      ) : (
