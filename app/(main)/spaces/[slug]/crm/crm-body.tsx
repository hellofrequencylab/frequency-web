...
// CRM BODY — the chrome-free per-Space CRM board, lifted out of the standalone /spaces/<slug>/crm page
// (Stage D5) so it renders in TWO places from one source: (1) that page, wrapped in its DashboardTemplate
// chrome + stats slot, and (2) INLINE in the Space profile body as the CRM `?panel=` workspace
// (components/spaces/workspace/space-body-panel.tsx). It owns NO page chrome (no DashboardTemplate, no
// eyebrow/title/description, no stats slot — the caller frames those) and SELF-GATES server-side so it is
// safe to mount anywhere: it returns null on a missing / not-visible Space, and a calm chrome-free locked
// notice when the viewer / plan cannot use CRM (the standalone page still 404s / renders its own LockedCrm
// via its own gate).
//
// SCOPE (unchanged from the page it was lifted from): it renders the board's DEFAULT view (People) plus the
// Pipeline + Cockpit views by `activeView`. The board's DEEP sub-navigation (?contact=, ?stage=) stays on
// the STANDALONE route — the panel keeps the default board and its view tabs link out to the full page, so
// no query-param threading collides with the profile's `?panel=` seam (Stage D5 report). Every read is
// fail-safe (lib/crm/pipeline.ts returns [] on error). No em or en dashes; voice per CONTENT-VOICE §10.
...
export async function CrmBody({
  ...
  slug: string
  /** The board view to render. Defaults to People (the panel + standalone front door). Pipeline / Cockpit
   *  are reached from the standalone route's ?view= tabs. */
  activeView?: CrmView
}) {
  ...
  // SELF-GATE (mirrors the standalone page's canUseCrm gate): the LIVE per-Space resolver folds the plan
  // entitlement + the per-Space min-role + the plan-ladder check. A viewer / plan that cannot use CRM gets a
  // calm chrome-free notice here (the panel header's "Open full page" opens the full LockedCrm with the
  // upgrade options + meter). The standalone page renders its own DashboardTemplate-framed LockedCrm, so it
  ...
  if (!canUseCrm) {
    ...
    return (
      <EmptyState
        ...
        description={
          hasCrm
            ? 'The CRM is for the people who run this space. Ask an admin to bring you onto the team.'
            : 'A CRM turns the people you meet into a pipeline you can work: stages, deals, and the contacts you bring over from My Contacts. Open the full page to see your plan options.'
        }
      />
    )
  }
  ...
  return (
    <>
      {/* LIST-FIRST (docs/NEXT-GEN-CRM.md): the persistent view tabs sit at the top of every board view,
          so the familiar People list is always one tap away from Pipeline or Cockpit. In the inline panel
          the Pipeline / Cockpit tabs link out to the standalone route (the deep views stay full-width). */}
...
// ListSkeleton is exported: the standalone page reuses it for its contact-detail + stage-drill fallbacks
// (the two deep sub-views that stay on the route), so the board's skeletons live in one place.
