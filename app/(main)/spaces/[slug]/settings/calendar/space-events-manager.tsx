...
export function SpaceEventsManager({ events }: { events: ManagedEvent[] }) {
  ...
  const btn =
    'inline-flex items-center gap-1 rounded-control border border-border px-2.5 py-1.5 text-meta font-semibold text-text transition-colors hover:border-primary-bg hover:bg-primary-bg/40 disabled:opacity-50'
  const dangerBtn =
    'inline-flex items-center gap-1 rounded-control border border-danger/40 px-2.5 py-1.5 text-meta font-semibold text-danger transition-colors hover:bg-danger-bg disabled:opacity-50'
        ...
        {events.map((ev) => {
          ...
          return (
            <RowCard
              ...
              badge={
                ev.isCancelled ? (
                  <span className="rounded-pill bg-danger-bg px-2 py-0.5 text-2xs font-semibold text-danger">
                    Cancelled
                  </span>
                ) : ev.isPast ? (
