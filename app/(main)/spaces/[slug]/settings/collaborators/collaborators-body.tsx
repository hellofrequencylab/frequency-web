  ...
  return (
    <div className="flex min-w-0 items-center gap-3">
      {partner.logoUrl ? (
        ...
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-border bg-surface-elevated text-body-sm font-bold text-subtle">
...
export async function CollaboratorsBody({
  ...
  if (!manage) {
    ...
    return (
      <p className="rounded-card border border-dashed border-border bg-surface px-4 py-6 text-body-sm text-muted">
    ...
    <div className="space-y-6">
      ...
      {incoming.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-body-sm font-semibold text-text">Requests to approve</h2>
      ...
      <section className="space-y-3">
        <h2 className="text-body-sm font-semibold text-text">Collaborators</h2>
        {accepted.length === 0 ? (
          <p className="rounded-card border border-dashed border-border bg-surface px-4 py-6 text-body-sm text-muted">
      ...
      {pendingSent.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-body-sm font-semibold text-text">Invites sent</h2>
          <ul className="space-y-2">
            {pendingSent.map((v) => (
              <li key={v.id} className="flex flex-wrap items-center justify-between gap-3 rounded-card border border-dashed border-border bg-surface p-4">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-meta font-medium text-subtle">Waiting for them to approve</span>
      ...
      {declined.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-body-sm font-semibold text-text">Declined</h2>
          <ul className="space-y-2">
            {declined.map((v) => (
              <li key={v.id} className="flex flex-wrap items-center justify-between gap-3 rounded-card border border-dashed border-border bg-surface p-4">
                <PartnerRow view={v} />
