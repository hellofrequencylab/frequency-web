    ...
    >
      ...
      <div className="space-y-6">
        <Link
          ...
          className="inline-flex items-center gap-1.5 text-meta font-semibold text-muted hover:text-text"
        >
    ...
    <div className="rounded-2xl border border-border bg-surface p-5 lift-1">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 space-y-1">
          <p className="text-body-sm font-bold text-text">{ui.title}</p>
          <p className="text-meta leading-relaxed text-muted">{ui.body}</p>
          {(orgLegalName || ein) && (
          ...
          {status === 'rejected' && note && (
            <p className="mt-2 rounded-lg bg-surface-elevated px-3 py-2 text-meta text-text">{note}</p>
          )}
        </div>
      </div>
    </div>
