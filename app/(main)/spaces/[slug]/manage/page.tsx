...
// circle console uses. This slice ships the `practitioner` and `organization` (nonprofit) type
// spines; other Space types stay on the existing /spaces/[slug]/settings 7-tab cockpit (NOT retired
// here — that harmonization is a later slice). No feature is rebuilt: each section LINKS to the
// existing settings sub-page that already serves it.
//
...
export default async function SpaceManagePage({
  ...
  // EM1-3 ships the practitioner + organization spines only. Any other type keeps using the legacy
  // /spaces/[slug]/settings cockpit, so the unified console is not offered for it: notFound() routes
  // the owner back to the working cockpit without revealing a half-built surface.
  if (space.type !== 'practitioner' && space.type !== 'organization') notFound()
