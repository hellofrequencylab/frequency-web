        ...
        <label key={f.key} className="block">
          ...
          {f.textarea ? (
            ...
            <input
              ...
              onChange={(e) => set(f.key, e.target.value)}
              placeholder={f.placeholder}
              className="w-full rounded-card border border-border bg-surface-elevated px-4 py-2.5 text-sm text-text placeholder:text-subtle outline-none focus:border-primary"
            />
          )}
        </label>
      ))}
