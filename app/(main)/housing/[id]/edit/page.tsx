...
export default async function EditHousingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const initial: HousingFormInitial = {
    ...
    laundry: detail?.laundry ?? null,
    bathroomsShared: detail?.bathroomsShared ?? false,
    moveInCostsDollars: detail?.moveInCostsCents != null ? Math.round(detail.moveInCostsCents / 100) : null,
    minStayMonths: detail?.minStayMonths ?? null,
    maxOccupants: detail?.maxOccupants ?? null,
    accessibility: detail?.accessibility ?? [],
    addressPrecision: detail?.addressPrecision ?? 'city',
