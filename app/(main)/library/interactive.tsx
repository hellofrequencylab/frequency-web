  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        className="inline-flex items-center gap-1.5 rounded-control bg-primary px-4 py-2 text-body-sm font-semibold text-on-primary lift-1 transition-colors hover:bg-primary-hover"
      >
        <Plus className="h-4 w-4" /> Create
        <ChevronDown className={`h-3.5 w-3.5 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <div role="menu" aria-label="Create" className="absolute right-0 top-full z-50 mt-1 w-44 rounded-card border border-border bg-surface p-1 shadow-pop">
