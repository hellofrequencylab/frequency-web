    ...
    <div className="-mx-6 -my-6 flex flex-col h-[calc(100vh-3.5rem)]">
      ...
      <header className="shrink-0 flex items-center gap-3 px-5 py-3 border-b border-border bg-surface">
        ...
        {isGroup ? (
          ...
          >
            {others[0].avatar_url ? (
              <Image width={36} height={36} src={others[0].avatar_url} alt={others[0].display_name} className="w-9 h-9 rounded-full object-cover shrink-0" />
            ) : (
            ...
            <ul className="flex-1 overflow-y-auto divide-y divide-border/50">
              {participants.map(p => (
                <li key={p.id}>
                  ...
                  >
                    {p.avatar_url ? (
                      <Image width={28} height={28} src={p.avatar_url} alt={p.display_name} className="w-7 h-7 rounded-full object-cover shrink-0" />
                    ) : (
