...
export function JoinButton({ token, circleName }: { token: string; circleName: string }) {
  ...
  return (
    <button
      onClick={handleJoin}
      disabled={isPending}
      className="w-full rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-on-primary hover:bg-primary-hover disabled:opacity-60 transition-colors flex items-center justify-center gap-2"
    >
