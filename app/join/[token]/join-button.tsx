...
export function JoinButton({ token, circleName }: { token: string; circleName: string }) {
  ...
  return (
    <button
      onClick={handleJoin}
      disabled={isPending}
      className="w-full rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white hover:bg-indigo-700 disabled:opacity-60 transition-colors flex items-center justify-center gap-2"
    >
