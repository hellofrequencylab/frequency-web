...
export const alt = `A partner on ${SITE_NAME}`
export const size = { width: 1200, height: 630 }
export const contentType = 'image/png'

// Per-partner dynamic OG image for /discover/partners/[slug].
// Falls back to a generic branded card when the partner isn't found.
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const partner = await getPartnerView(slug).catch(() => null)

  const name = partner?.name ?? 'Partners'
  const city = partner?.city ?? null
  const category = partner?.category ?? null
  const isFallback = !partner
    ...
    (
      ...
      >
        ...
        <div style={{ display: 'flex', fontSize: 26, color: 'rgba(255,255,255,0.72)' }}>
          {isFallback ? `Community partners on ${SITE_NAME}` : `A community partner on ${SITE_NAME}`}
        </div>
      </div>
    ),
    size,
