  ...
  return (
    <ModuleSection anchor="team">
      <div>
        ...
        <p className="font-eyebrow text-2xs font-bold uppercase tracking-eyebrow text-primary-strong">{header?.eyebrow ?? 'The people'}</p>
        ...
        <ul className="mt-6 grid gap-4 [grid-template-columns:repeat(auto-fill,minmax(11rem,1fr))]">
          {team.map((member) => (
            <li
              ...
              className="flex items-center gap-3 rounded-card border border-border bg-surface p-3 lift-1"
            >
              {member.avatarUrl ? (
                <Image
