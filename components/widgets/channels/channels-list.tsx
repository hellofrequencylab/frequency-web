...
// Channels layout module (ADR-270/294): the pillar-grouped browse of the topical Channels — the
// four Pillars (Mind, Body, Spirit, Expression), each with its Channels split into tuned-in vs
// explore, plus the on-page table-of-contents and the Pillars jump-nav. A self-fetching RSC keyed
// only on the viewer (their tuned-in set drives the split), so it converts wholesale to one module
// with no searchParams facet. The "No Channels yet" empty state stays part of the block, so an
// operator who places it always shows a member something, never a blank surface (the module
// contract). The page keeps only the IndexTemplate header + create/CTA action.
...
// Tune-in remains the per-topic action; the tuned-in vs explore framing is preserved within each.
export async function ChannelsList() {
  ...
  // Group the Interests under their Channel (Domain). Anything not yet assigned to a domain falls
  // into an "Unsorted" bucket at the end so it stays visible until an admin assigns it.
  ...
  // Table-of-contents links — one chip per Channel (Domain) that jumps to its on-page section. We
  // use the FILTER variant of PageContents (like Circles) so the network stats can ride at the bar's
  // right with no divider rule. The "All" chip leads and is the resting/active state (this is a
  // single long page, not a URL-filtered view, so nothing else is "active").
  const tocLinks = [
  ...
  return (
    <div>
      ...
      {/* Table of contents — jump between Channels. Per-Pillar counts ride on each chip; taxonomy
          counts stay quiet inline context, never KPI tiles (the gamified-stat law,
          MEMBER-DESIGN-SYSTEM §2). */}
      <PageContents links={tocLinks} divider={false} />

      {/* Two columns: the Channels flow on the left; the browse nav sits in a STABLE right column
          so it's never orphaned at the bottom. */}
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_17rem]">
        ...
        {/* Right: the browse nav — the Channels list, pinned at the top of its column (the
            at-a-glance stats moved up beside the filter menu). */}
...
// A Channel (Domain) section: its name + description, then its Interests, split into tuned-in vs
// explore so the framing carries through to each Channel.
