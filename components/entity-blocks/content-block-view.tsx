    ...
    body = (
      <div className="flex flex-col gap-5">
        {items.map((it, i) => {
          ...
          return it.link ? (
            <a key={i} href={it.link} className={`${rowCls} rounded-card transition-colors hover:bg-surface-elevated`}>
              {inner}
            </a>
          ) : (
        ...
        <ul className="flex flex-col gap-2">
          {links.map((it, i) => (
            <li key={`${it.url}-${i}`}>
              <a
                href={it.url}
                className="flex items-center justify-between rounded-card border border-border bg-surface px-4 py-3 text-sm font-semibold text-text transition-colors hover:border-primary hover:text-primary-strong"
              >
                {it.label || it.url}
              </a>
            </li>
