    ...
    <div className="grid grid-cols-1 gap-2 lg:grid-cols-3 lg:items-center lg:gap-6">
      <div ref={boxRef} className="relative min-w-0 lg:col-span-2">
        ...
        <input
          ...
          className="w-full rounded-card border border-border bg-surface py-2.5 pl-9 pr-9 text-sm text-text placeholder:text-subtle focus:border-border-strong focus:outline-none focus:ring-2 focus:ring-border-strong/30"
        />
        {(loading || q) && (
        ...
        {open && results.length > 0 && (
          <ul className="absolute z-20 mt-1 w-full overflow-hidden rounded-card border border-border bg-surface lift-3">
            {results.map((r) => (
              <li key={r.label}>
                <button
                  type="button"
      ...
      <div className="flex shrink-0 items-center gap-2">
        <button
          ...
          disabled={locating}
          className="inline-flex shrink-0 items-center justify-center gap-1.5 rounded-card border border-border bg-surface px-3.5 py-2.5 text-sm font-medium text-text transition-colors hover:border-primary hover:text-primary-strong disabled:opacity-60"
        >
