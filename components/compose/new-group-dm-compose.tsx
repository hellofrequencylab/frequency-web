            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {recipients.map(r => (
                <span key={r.id} className="inline-flex items-center gap-1 rounded-md bg-primary-bg border border-primary-bg px-2 py-1 text-xs">
                  {r.avatar_url ? (
                    <Image width={16} height={16} src={r.avatar_url} alt={r.display_name} className="w-4 h-4 rounded-full object-cover" />
                  ) : (
...
function ResultRow({
  ...
  return (
    <div className={`flex items-center gap-2.5 w-full px-3 py-2 ${isFriend ? 'hover:bg-surface-elevated transition-colors' : 'opacity-60'}`}>
      {result.avatar_url ? (
        <Image width={28} height={28} src={result.avatar_url} alt={result.display_name} className="w-7 h-7 rounded-full object-cover shrink-0" />
      ) : (
