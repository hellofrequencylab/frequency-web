...
export function PageAdminBar({ asDivider = false }: { asDivider?: boolean } = {}) {
  ...
  // The circle DETAIL page owns its own settings entry now ("Edit Circle" in the header, which opens
  // the same drawer), so this bar's duplicate Settings trigger is suppressed there — QR & Share stays.
  ...
  const settingsTrigger = (manager || isOperator) && !isCircleDetail ? (
    <button
      type="button"
