  ...
  return (
    <div className="w-full max-w-sm">
      ...
      {streak.freezeTokens > 0 && (
        <p className="mt-4 flex items-center justify-center gap-1.5 text-meta font-medium text-muted">
    ...
    <div className="w-full max-w-sm">
      ...
      {nudge && (
        <div className="mt-3 rounded-xl border border-primary/40 bg-primary-bg/30 px-3 py-2 text-left">
          ...
          {nudge.toNext ? (
            <p className="mt-0.5 text-meta text-muted">{nudge.toNext}</p>
          ) : (
            <p className="mt-0.5 text-meta text-muted">Top of the dial. Hold it here.</p>
          )}
        </div>
      )}
      {depthFlavor && (
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <div className="rounded-2xl border border-primary/50 bg-primary-bg/30 p-5 text-left lift-1">
        ...
        <p className="mt-3 min-h-[3.5rem] text-body font-medium leading-relaxed text-text">
        ...
        {dispatch.actionHref &&
          (actionIsFeed && onClose ? (
            ...
            <button
              type="button"
              onClick={onClose}
              className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-primary px-3.5 py-2 text-body-sm font-semibold text-on-primary transition-colors hover:bg-primary-hover"
            >
        ...
        {!actionIsFeed &&
          (onClose ? (
            <button
              type="button"
              onClick={onClose}
              className="text-meta font-semibold text-subtle hover:text-text"
            >
              Back to feed
            </button>
          ) : (
            <Link href="/feed" className="text-meta font-semibold text-subtle hover:text-text">
              Back to feed
            </Link>
          ))}
      </div>
