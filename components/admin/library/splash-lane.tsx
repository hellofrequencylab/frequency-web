...
function StatusChip({ status }: { status: string }) {
  ...
  return (
    <span className={`shrink-0 rounded-pill border px-2 py-0.5 text-2xs font-semibold uppercase tracking-wide ${tone}`}>
      {status}
    </span>
  )
}
          ...
          <div className="overflow-hidden rounded-2xl border border-border">
            {liveRows.map((l) => {
              ...
              return (
                ...
                >
                  <span className="inline-flex shrink-0 items-center gap-1 rounded-pill border border-border bg-surface-elevated px-2 py-0.5 text-2xs font-medium text-muted">
