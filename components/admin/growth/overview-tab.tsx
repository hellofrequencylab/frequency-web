...
import { groupLinks, type AdminLink } from '@/app/(main)/admin/sections'
...
// The "Overview" tab of the consolidated Growth workspace (ADR-264) — formerly the
// /admin/growth roll-up dashboard. The same tiled grammar (KPI clusters + graphs + a
// domain attention strip + area-card entry tiles), now the default tab. Each heavy
// aggregate sits behind its own Suspense so the shell never blocks (PAGE-FRAMEWORK §5).
...
async function PipelineArea() {
  ...
  if (metrics.tasksDue > 0) {
    attention.push({
      ...
      action: { label: 'Open CRM', href: '/admin/growth?tab=crm' },
    })
  }
