...
function EventSettingsRail({ data, engage }: { data: EventData; engage: EventCoreStats | null }) {
  ...
  const saveSettings = async () =>
    updateEventSettings(eventId, eventSlug, eventSettingsFormData(valuesRef.current, pinRef.current, scopeRef.current))
      ...
      <RailAutosaveForm action={saveSettings} className="space-y-4">
        ...
        {GROUPS.map(({ section, fields }) => (
          <div key={section.key} className="space-y-3 rounded-card border border-border bg-canvas/40 p-3">
            ...
            <RailManifestFields fields={repeatGated(fieldsFor(fields))} values={values} onChange={update} placeholders={PLACEHOLDERS} hints={hints} />
            {/* 🔴 THE REPEAT EDITOR IS REMOVED, NOT DISABLED, UNDER THE NARROW SCOPE — and it says
                ...
                refused by the server is the failure this whole ADR is about. */}
