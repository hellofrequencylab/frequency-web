...
import { useSpaceRailCore, useSpaceRailSlice } from './space-rail-data'
...
export function SpaceBrandingModule() {
  ...
  // Slice from the fast CORE phase (ADR-550 two-phase); self-fetch fallback keeps it working standalone.
  const { data, loading } = useSpaceRailSlice(slug, (b) => b.branding, getSpaceBrandingData)
  // The hero LOOK (height + button orientation) is a PURE seed carried on the same fast core (item 5 moved
  // its editing here). Standalone (no provider) it is undefined, so the form falls back to its defaults.
  const core = useSpaceRailCore()
  const heroLook = core?.status === 'ready' ? core.data?.hero : undefined
