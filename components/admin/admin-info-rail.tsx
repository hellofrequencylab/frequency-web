...
function InfoStat({ label, value, href, icon: Icon }: { label: string; value: string | number; href: string; icon: LucideIcon }) {
  return (
    <Link
      href={href}
      className="flex items-center justify-between rounded-card px-3 py-2.5 transition-colors hover:bg-surface-elevated"
    >
        ...
        <div className="space-y-0.5">
          {d.newest.map((m) => (
            <Link
              ...
              className="flex items-center justify-between rounded-card px-3 py-2 transition-colors hover:bg-surface-elevated"
            >
