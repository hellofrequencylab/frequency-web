...
import { dashCookie, sanitizeDashOrder } from '@/app/(main)/admin/dash-sections'
...
// The Overview tab of the Programs workspace: the domain at a glance (ADR-233 §3.2).
// The catalog volume + library trend and the live season + outcomes, in the tiled
// white-card grammar, with the operator's saved section order (the page dock organizer,
// ADR-129). Each slow read streams behind its own Suspense (PAGE-FRAMEWORK §5).
...
export async function OverviewTab({
  ...
  return (
    <>
      {order.map((id) => (sections[id] ? <Fragment key={id}>{sections[id]}</Fragment> : null))}
...
async function CatalogArea() {
  ...
  if (pendingReviews > 0) {
    attention.push({
      ...
      action: { label: 'Review', href: '/admin/programs?tab=content' },
    })
  }
