...
export function SpaceBusinessForm({
  ...
  // Services (the storefront store items) are edited on their OWN surface now (/settings/services,
  // SpaceServicesForm), which carries their full pricing + visibility. This form deliberately does NOT
  // touch offerings, so saving business info never strips a service's pricing fields.
  ...
  function save() {
    ...
    start(async () => {
      // NOTE: no `offerings` key here on purpose. Services are owned by SpaceServicesForm; omitting the
      // key lets withProfileData preserve the stored catalog (and its pricing) untouched on this save.
