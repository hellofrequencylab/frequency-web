...
export async function OwnerSpaceLayoutPreview({ slug }: { slug: string }) {
  ...
  // The on-page photo popup is Loom-only (owner directive): the grid derives the Loom scope from `spaceSlug`
  // below, and the Loom's own actions re-gate every read + write, so this preview injects no upload path.
