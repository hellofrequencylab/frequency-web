  ...
  return (
    <div>
      ...
      <div className="mt-1.5 flex flex-wrap gap-1.5">
        {items.map((item) => (
          <span key={item} className="rounded-pill bg-surface-elevated px-2 py-0.5 text-2xs font-medium text-muted">
            {item}
          </span>
        ))}
      </div>
...
function TimelineRow({ entry }: { entry: TimelineEntry }) {
  ...
  return (
    <li className="flex gap-3 rounded-card border border-border bg-surface p-4 lift-1">
