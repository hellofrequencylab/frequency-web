...
export function CalendarPmConsole({
  ...
  const items = operatorListItems(events).filter((row) => row.stageLabel !== 'Planning')
  return (
      ...
      <section aria-labelledby="calendar-pm-planning" data-planning-lane>
        ...
        {planning.length === 0 ? (
          <EmptyState
            ...
            description="The date is decided. The team is putting it together on the date map below."
          />
        ) : (
      ...
      <section aria-labelledby="calendar-pm-board">
        ...
        {items.length === 0 ? (
          <EmptyState
            ...
            description="Pencil, Production, and Cancelled land here. Planning has its own lane above."
          />
        ) : (
