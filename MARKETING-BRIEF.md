...
| **Outpost / Region** | Structural ancestry in the breadcrumb (de-emphasized) | n/a |
...
**Adopt** (someone takes a Channel's program, opens a new circle, and becomes its host).

### Member-facing vocabulary (deliberately tiny)
A member only ever needs **two belonging-words: Circle (my group) and Channel (my
topic).** Everything structural (Hub, Nexus) is met contextually as *places and events*,
never as nav. Above the Channels sit the four **Pillars** (Mind, Body, Spirit, Expression),
the top organizing layer.

**The topics** (7 global Channels): **Spirituality, Movement, Holistic Health, Human
...
> ✅ **Naming and voice are locked. The two sources of truth are [`docs/NAMING.md`](docs/NAMING.md)
> (terminology) and [`docs/CONTENT-VOICE.md`](docs/CONTENT-VOICE.md) (voice + punctuation);
> defer to them on every word.** **The Quest** = the game; a **Quest** = one season; the engine
> hierarchy is **Quest, Journey, Practice**. **Zaps/Gems** = the currencies (never "points");
> ranks are **Ghost, Echo, Signal, Beacon, Conduit, Luminary**; depth tiers are
> **Initiate / Adept / Master**. **Circle** is the member word. The topic level is settled:
> the four **Pillars** (Mind, Body, Spirit, Expression) are the top layer, and the seven
> **Channels** (verb "tune in") are the topics inside them. "Interest" is retired as a
> member-facing word for the topics.
...
- **Channels** (topical spaces with discussion + memberships)
...
1. Sign up, then **pick Channels** (pure virtual)
2. **Join a Circle** *or* **adopt a Channel's program** (and become a host)
