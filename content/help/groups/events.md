...
- **Your Space calendar is a manager.** On the Space Calendar tab you switch five views: **Guest** (what visitors see), **Admin** (the production board and the date map; Pencil, Planning, and Production are their own lanes), **List** (every gathering on the left, stats and management on the right), **Timeline** (the month as a time scale), and **Projects** (move an event on its way through Pencil, Planning, Production, and Cancelled). Guest is live events only, and a cancelled gathering as a small note on that day (and under that month in the list). Dates that are only penciled or still in planning stay off Guest. In Calendar settings you still flip the month grid and a list, click any event to edit it, and handle co-host requests. Drafts and past events show there too.
