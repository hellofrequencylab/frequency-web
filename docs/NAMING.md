...
- **Trophy** = the award minted when a member **finishes a Journey** (it advances rank and
  pays the escalating Gem bonus; supersedes the old flat 30-Gem journey reward).
...
- **Economy:**
  ...
  - At season end, **Zaps roll into Gems FLAT at 5:1** (floor division, `ZAP_TO_GEM_RATIO = 5`).
    The old one-time **final-rank Gem bonus is RETIRED** — its reward now rides the
    **per-Journey Trophy**: finishing a Journey pays **+75 Zaps + a Trophy + an escalating
    Gem bonus by the rank reached (Initiate 25 / Adept 50 / Master 100)**. The Expression
    Challenge that caps a Journey pays **+50 Zaps in person at a Circle, or +30 Gems posted
    solo online**, and is required to finish the Journey. A Journey is finished when the member
    has logged ≥1 of its Practices on **14–16 distinct days inside its ~4-week window** AND
    completed its Expression Challenge.
...
- **"luminary"**: fully retired as a season rank; the recruiter-ladder apex that
  reused it is renamed **Catalyst** (`lib/entry-points/leaderboard.ts`).
