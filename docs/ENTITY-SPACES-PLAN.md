...
**Per-vertical table-stakes + the differentiators we can win on** (from vertical research; each has hard compliance gates — treat as table-stakes, not nice-to-haves):
...
1. **Blueprint per `type`** — a seed definition: which `enabled_verticals[]`, which modules per page, default theme `skin`, default circles/pages, sample content. Stored as data, not code.
...
- **Keep platform/operator admin separate** (the `web_role` staff axis stays global and locked — same boundary we just shipped for beta access).
...
- "Remove Frequency branding" + **branded notification email** (from the space's domain) as a tier feature — mirrors Circle Business / Heartbeat.
...
- **Tenant-scope every cache key** (CDN, Redis, Next.js data cache); mark space-private pages `no-store`. A cache key that omits the space is the classic invisible cross-tenant leak (the Railway-class incident). Audit every `service_role`/`SECURITY DEFINER` path — that's where leaks hide.
- **Theming with no flash:** because the space is resolved server-side from the host, render its semantic-token CSS variables straight into the space layout's HTML (inline `style`/`<style>`), not from `localStorage` — server and client markup match, so there's zero FOUC and no blocking script. Keep it all in the existing token system (no hardcoded hex).
...
- **Free/Starter space:** networked space, basic circles/events/practices, 1–2 QR codes, Frequency-branded URL.
...
> Note: Frequency's access matrix already has a **`hookNetwork`** surface labeled *"Org product"* — the two were apparently designed to interoperate, which slightly favors (A)/(B). Confirm on read.
...
- **Scope creep across four verticals** — booking, ticketing, donations, memberships are each deep. **Ship one vertical end-to-end first** (recommend **Practitioner** — smallest surface, clearest value), then template the next.
...
| **6 — Premium** | Native apps (done-for-you) | branded app program as a top tier — only after 1–5 are proven |

Verticals roll out **one at a time** on top of Phase 0–1: Practitioner → Business → Event Space → Non-Profit, each adding its blueprint + the one or two deep features it needs (bookings, ticketing/check-in, donations).
