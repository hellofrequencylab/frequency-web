...
|---|---|
<<<<<<< HEAD
| **You cannot sell a course.** `journey_plans` has no price, no Stripe product, no tier link. | ✅ Shipped as a Journey commerce product (`product_kind = journey`, ADR-1397). Price lives on the product row, not on `journey_plans`. |
| **No gated content except events.** Only `event_ticket_types` carries `member_only` / `space_tier_id`. Journeys were `private`, `unlisted`, or `public`, with no fourth "members of my tier" state. | ✅ `journey_plans.space_tier_id` (LIVE-411, ADR-1472). Visibility is still who can find it. |
