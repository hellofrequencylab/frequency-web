...
- **In-browser editor:** **Filerobot Image Editor** (OSS) — crop with aspect frames, rotate,
  ...
  ([HYG-108](BUILD-BACKLOG.json), [ADR-1493](DECISIONS.md)). This was picked before anyone measured
...
- **One master, many renditions, resolved at REQUEST time** ([ADR-1493](DECISIONS.md), executing the
...
2. **D2 — AssetField seam** (unified picker; store references; render resolution; backfill
   `site-media`).
3. **D3 — Editor + versions.** Shipped and closed ([ADR-1493](DECISIONS.md)): version-on-edit and
