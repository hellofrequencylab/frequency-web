...
| **Art. 50 AI transparency** (disclose Vera + all AI-generated copy as AI) | ✅ Shipped (OWN-061, [ADR-1515](DECISIONS.md)). The duty applied from **2 Aug 2026** and is met by one component, `components/vera/ai-disclosure.tsx`, rendered on every member-facing Vera surface: the companion chat, the onboarding lightbox, the Studio review board, the Dispatch, the events lane, Drafts, and the Circle builder's Vera panel. The test beside the component lists those surfaces, so a new one is added there or it fails. (Synthetic-media watermarking by Dec 2026 is still ahead.) |
