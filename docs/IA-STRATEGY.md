...
> **Names:** [NAMING.md](NAMING.md) always wins. The member-facing topical noun is
> **Channel**. **"Interests" is retired.** §6 once decided the opposite; that ruling
> is reversed. Product nouns: [CORE-MODEL.md](CORE-MODEL.md).
>
...
> 2026-06-05 spec below where they differ. **Phases 1 to 3 are shipped** (PRs #301 /
...
```
◆ HOME        Feed
◆ COMMUNITY   Circles · Channels · Events        (Channel = the topical noun; Interests retired)
...
- **`Channels` is the member-facing topical noun** ([NAMING.md](NAMING.md)). "Interests" is retired; do not nest a second topical label inside Channel.
...
```
  ...
  Channels                     ← topics you practice (NAMING.md; never "Interests")
  Events
...
person**, and those carry a small in-person **icon designator** *and* a **tighter
...
Your instinct, that the whole site is `top bar + left nav + right "what's going on"`,
...
- **Discipline:** 3 to 5 cards visible max. Cognitive-load limits apply to
  sidebars too. Each card is scannable and links deeper.

---

## 6. Naming: "Interests" is retired (reversed)

This section originally decided a member-facing rename **Channels → Interests**.
**That ruling is reversed.** [NAMING.md](NAMING.md) (ADR-208) is canon:

- **Member-facing noun: Channel** (`topical_channels`). Verb: "tune in."
- **"Interest" / "Interests" is retired** as a word for these. Do not relabel
  `/channels`, nav, or onboarding to Interests.
- The schema overload this section was written about is gone: the legacy
  `channels` table was dropped, empty, on 2026-09-15 ([ADR-1349](DECISIONS.md)).
  `/admin/channels` manages `topical_channels`.

Keep this heading so old links still land on the reversal, not a second canon.
...
| 8 | ~~Relabel Channels → Interests~~ **Do not.** NAMING.md keeps Channel. | — | reversed |
...
- **Belonging words = Circle + Channel.** "Interests" is retired ([NAMING.md](NAMING.md)).
...
- **Nexus**: the *whole area*: gathers quarterly for galas, big events. Designed to
...
- ✅ **§6 naming**: reversed. Channel is canon; Interests is retired. A Channel
  is also the program catalog + circle spawn point.
