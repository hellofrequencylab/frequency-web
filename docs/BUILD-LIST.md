...
| "no rollback convention anywhere" | **17 of 607 migrations carry a literal `-- ROLLBACK:` block; 114 carry some hand-reversal note** (`rollback` or `reversible`, any case). Schema has a convention; the *render path* has none. ⚠️ Re-measured 2026-08-12: this row read "49 of 596", and **49 matches no pattern in the tree** — the count is stated with its grep so the next reader can re-derive rather than inherit |
