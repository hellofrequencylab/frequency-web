...
|---|---|---|---|
| **P-a** | [`FINALIZE-PLAN.md`](FINALIZE-PLAN.md) Phase 1.2/1.3 — recapture the visual baselines from a settled `main` | ⏳ **IN FLIGHT — PR #2071** | 62 of 72 refreshed against the current design (they were failing on same-size pixel diffs, ratios 0.03–0.17 against a 0.02 tolerance — the design moved). ⚠️ **Two things it does not finish:** the member shell is still 2 of 4 surfaces pending two repo variables (`PW_ROOM_PATH`, `PW_SPACE_SLUG` — an owner action), and **none of E3's four target surfaces gains a baseline**, which independently confirms [§8.1 item 2](EDITOR-ARCHITECTURE.md). ⚠️ That PR also found the **a11y ratchet is stale for the same reason and takes the OPPOSITE remedy** — 28 of its failures are real rises (all 16 dark-mode contrast checks), and recapturing a ratchet deletes the finding |
