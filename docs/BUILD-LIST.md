...
- **2026-06-13 ✅ Naming Canon 2026 (ADR-208)** — locked the platform vocabulary in
  [`NAMING.md`](NAMING.md) (the single source of truth) and aligned the schema: **Domains → Pillars**
  (`pillars`, `topical_channels.pillar_id`), depth tiers **Spark/Current/Deep → Initiate/Adept/Master**
  (default Adept), season ranks **Runner/Operative/Agent → Echo/Signal/Beacon**, **Circle Field →
  Circle Current** (`circles.season_current`, `circle_current_transactions`), **Chorus → Co-op**, and a
  two-axis role split (`profiles.web_role` alongside `community_role`). Migrations `2026061300*`
  (data-preserving, idempotent); thresholds/economy/RLS substance unchanged. Retired: Arc/quest_chains
  (engine dropped), "Seasonal Quest" phrasing, deshi/sempai/sensei, "points." Technical docs rewritten
  to canon this pass.
