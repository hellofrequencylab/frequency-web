...
| 2.6 | Freemium Vault + season cash-in | ⏳ | Accrual (zap/gem ledgers) ✅, season `zaps → gems` conversion ✅, persistent Vault + "how you earned" log ✅. **Lifetime rank** ✅ — locked monotonic peak (`profiles.lifetime_rank`, ADR-164/migration `20260608060000`): zap trigger ratchets it, `reset_season()` preserves it, surfaced on the Vault. **Cash-in eligibility** ✅ (ADR-225) — pure `canCashIn(tier) = isPaid(tier)`; `redeemItem` now enforces it **server-side** (free members get a clean `/upgrade` upsell; accrual stays free, only claim/spend is paid). *Remaining:* entitlement **sources** beyond `membership_tier` — host comp-grant / Lab rollup / staff grant (ADR-037 §6c/d; speculative beta infra); optional RLS cash-in floor on `store_redemptions`. |
