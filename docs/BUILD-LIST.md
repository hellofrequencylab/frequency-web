...
| **1.6** | Unified capability resolver | ⏳ | §11.6 — one resolver = union of Community edges ⊕ Entitlement ⊕ Partner personas ⊕ Admin matrix. **Foundation shipped** (ADR-220, additive/behavior-preserving): the per-scope resolver (`capabilities.ts`) now reads `stewardships` edges via a `leadsScope` predicate **OR'd with the leader FK** (provably a no-op on the backfill data — every FK has a matching edge; scope-isolated by construction); edges fetched once/request in `load-capabilities.ts`; parity test locks no-behavior-change; `requireScopedManage` re-bases for free. Org-tenant isolation satisfied by absence (no tenant infra yet — deferred until Hook tenancy). **Remaining (behavior-changing, needs sign-off):** surface matrix scope-aware (`getViewerHats` → `community_level`, scoped `surfaceAccess`) + loosen the parent-walk to an edge gate — staged behind a flag + shadow report. |
