...
| P9 | **Marketing ↔ in-app reconciliation.** ✅ Re-measured 2026-08-25 ([ADR-1121](DECISIONS.md)) and the premise was wrong: there is ONE token system, not two. Marketing carries **0 raw hex / 0 arbitrary type across 61 files** and uses a strict subset of the same `@theme` map; what diverged was the **gates**. `check:tokens` exempted the whole marketing tree (measured: the exemption caught nothing) and `check:contrast` modelled the cream ground with **1 pair against the app canvas's 5**. Both closed. The Anton display face, the `.mk-*` rhythm and `--color-marketing-canvas` are deliberate and were NOT flattened. | ~~M~~ S | ✅ |
