...
- **2026-06-08 ✅ Agentic support seed (PI.4 follow-on)** — an **AI draft** button in the support console (`/admin/support/[id]`): `draftReply` (host+) has Claude draft a warm, on-tone reply from the ticket thread, returned to the agent to **edit and send** (never auto-sends). Budget-gated (`support-draft`) with a deterministic fallback when AI is off. The first step of the "virtual staff fields support through Claude" vision (ADR-167); help-article grounding is the next follow-up. 443 tests green.
