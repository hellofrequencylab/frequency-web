...
> UX plan both quote from an older column — `literal-radius` is baseline **2,440**, current **2,288**
...
| "no rollback convention anywhere" | **17 of 609 migrations carry a literal `-- ROLLBACK:` block; 115 carry some hand-reversal note** (`rollback` or `reversible`, any case). Schema has a convention; the *render path* has none. ⚠️ Re-measured 2026-08-12 (second pass): this row read "49 of 596" and then "17 of 607"; **49 matches no pattern in the tree**, and the denominator drifted again within the same day. Re-derive, never inherit — `ls supabase/migrations/*.sql \| wc -l` → **609**, `grep -rlE '^\s*--\s*ROLLBACK:' supabase/migrations/*.sql \| wc -l` → **17**, `grep -rliE 'rollback\|reversible' supabase/migrations/*.sql \| wc -l` → **115** |
...
|---|---|---|---|
| **P-a** | [`FINALIZE-PLAN.md`](FINALIZE-PLAN.md) Phase 1 — recapture the visual baselines | ✅ **DONE 2026-08-10 (PR #2071)** | **68 of 72** recaptured. `pr-compare` went from **62 failures to 1** on this repo's next PR. ⚠️ **Two things it does not close.** The remaining failure is `/feed` refusing to settle between consecutive captures (a live content surface — 2 of 3 runs passed on retry), so the suite is green-except-one-flaky rather than green. And **none of E3's four target surfaces gained a baseline**, so E3's "zero visual diff" gate is still unmeasurable — §8.1 item 2, still unowned. ⚠️ It also found the **a11y ratchet is stale for the same reason and takes the OPPOSITE remedy**: 28 of its failures are real rises (all 16 dark-mode contrast checks), and recapturing a ratchet deletes the finding |
...
| **Equivalence** (five) | old ⇄ new renderer · golden markup per (block × surface × density), generated from the registry · **email golden strings** (two cron send paths — `nurture` and `space-campaigns`; `space-drips` never touches a block document — and email has no visual gate at all, while sending 297 emails in the last 30 days) · RSC ⇄ canvas parity · **CRDT ⇄ tree round-trip** — without it the other four validate an artifact the editor does not produce |
| **Runtime** | `platform_flags.render_path`, per surface. `platform_flags` carries dozens of switches — AI, demo mode, SMS, referrals, feed, billing, every plan gate — and **not one** reverts a surface to its coded body (no seeded key matches `render`/`template`/`block`/`editor` except `circle_templates_enabled`, which is unrelated); **17 of 609 migrations carry a literal `-- ROLLBACK:` block and 115 carry some hand-reversal note (re-derived 2026-08-12, commands in the §P0 row above), so schema changes have a convention — the render path has none at all**, and this flag is what gives a rendering phase what a schema phase already has |
...
| P5 | **Entity headers → `PageHero`.** Fold the `DetailTemplate` band pages (measured 30 by render site, not 43; ADR-1498) onto the one `PageHero` grammar (entity + index = one component). The Space profile hero and the event poster band stay off it by ruling (`LIVE-447`). | M–L | 📋 |
...
| **Guestbook** | Visitors leave a note on your Spotlight. Needs a `spotlight_guestbook` table (RLS: owner reads all, anyone-signed-in writes), moderation (hide/report, owner delete), rate-limit + anti-spam, and the read-side render. | ✅ **SHIPPED** 2026-08-25 ([ADR-1132](DECISIONS.md)): migration `20270325000000_spotlight_guestbook.sql` (one note per signer per book; hide keeps the slot), `lib/spotlight/guestbook{.shared,}.ts`, session-client actions in `app/spotlight/[handle]/guestbook-actions.ts` (hourly cap), `guestbook` member grid block. Depth shipped 2026-09-08 ([ADR-1279](DECISIONS.md)): a note is a `reports.target_type` (migration `20270345002600`), the owner unhides from a Hidden notes list, and a sign notifies the owner behind their Replies switches |
| **Stickers / decals** | A playful decorative layer — place emoji/earned stickers on the page (absolute-positioned, validated coordinates + an allowlisted sticker set). | ✅ **SHIPPED** 2026-09-08 ([ADR-1275](DECISIONS.md)): page chrome at `meta.spotlight.stickers`, closed allowlist `lib/spotlight/stickers.ts` (the `requiredItem` seam declared, unused), `validateSpotlightStickers` on read and write, `setSpotlightStickers` session-derived, `components/spotlight/sticker-layer.tsx` in `SpotlightShell`, a Stickers section in the appearance rail. Earned stickers landed 2026-09-08 ([ADR-1279](DECISIONS.md)): `lib/awards/holdings.ts` + `lib/spotlight/cosmetics.ts` gate both `requiredItem` seams on write and in the picker; the Spectrum sticker (the Full Spectrum award) is the first earned row |

Bandcamp, Apple Music and Twitch joined the embed allowlist on 2026-09-08 ([ADR-1279](DECISIONS.md)):
each is three switch arms in `lib/spotlight/embeds.ts` plus its player host in `frame-src`, and
`embeds.test.ts` holds the two lists together. Earned skins share the sticker gate; no skin carries
an item yet.
...
| 2.6 | Freemium Vault + season cash-in | ⏳ | Accrual (zap/gem ledgers) ✅, season `zaps → gems` conversion ✅, persistent Vault + "how you earned" log ✅. **Lifetime rank** ✅ — locked monotonic peak (`profiles.lifetime_rank`, ADR-164/migration `20260608060000`): zap trigger ratchets it, `reset_season()` preserves it, surfaced on the Vault. ~~**Cash-in eligibility** ✅ (ADR-226) — pure `canCashIn(tier) = isPaid(tier)`; `redeemItem` enforces it server-side.~~ 🔴 **REVERSED 2026-09-09** ([ADR-1295](DECISIONS.md), owner ruling, `OWN-071`): `canCashIn`, the `vault_cash_in` and `gamification_full` gates and the Vault's ✋ matrix row are all deleted. The Quest is a side thing we all do together, so any signed-in member may spend the Gems they earned. *Remaining:* entitlement **sources** beyond `membership_tier` — host comp-grant / Lab rollup / staff grant (ADR-037 §6c/d; speculative beta infra); optional RLS cash-in floor on `store_redemptions`. |
...
| 3.3 | Practitioner | 📋 | **Paywalled Programs — the substrate is back, so this is a real feature again.** Corrected 2026-08-17: this row said Programs "names a feature with no substrate", which was true of the interim window and is not true now. Per [ADR-864](DECISIONS.md), a Program is a `topical_channel` with a blueprint, and per ADR-870 a Space can own one (`owner_space_id`). So the paywall has something concrete to attach to — the open question is no longer "is there a substrate" but **what exactly is gated**: starting a Chapter, access to the blueprint, or the Program channel's feed. That is a product call, not a re-scope. Client gamification + private Channel/Circles (Frequency-branded) + Connect are unaffected and stay 📋. |
...
| # | Item | Status |
|---|---|---|
| 4.1 | **Programs.** ✅ **The re-scope this row asked for SHIPPED — see [ADR-864](DECISIONS.md) + [ADR-870](DECISIONS.md).** Corrected 2026-08-17, and this row has now been stale in *both* directions, which is the reason status moved to [`BUILD-BACKLOG.json`](BUILD-BACKLOG.json). **What died:** the markdown-framework library. `content/programs/` is gone, `20261113000000` dropped the `program` arm from the `community_library` catalog RPC and `20261114000000` dropped the tables. **What lives:** a Program is now **a `topical_channel` carrying a blueprint** — `Program = topical_channels.template_id set` · `Chapter = a circle in that channel` · `owner_space_id NULL = Frequency-run, set = a Space runs it` (`20270110000000_programs_on_channels.sql`). `lib/channels/programs.ts` exports `isProgram` under test, `lib/circles/tabs.ts` documents the Chapter relationship, blueprints got their own discriminator in `20270113000000` (ADR-870), and `app/(main)/spaces/[slug]/settings/program/page.tsx` is the operator surface. **Do not schedule a "Programs library" and do not re-scope: both already happened.** | ✅ shipped |
...
**Practice / Quest / Gamification** (BACKLOG §F): ✅ daily-streak achievement badges (2026-06 — `practice_streak` criteria over `profiles.current_streak`, evaluated on each log, pays zaps; 3 badges seeded: Week of Devotion 7d · Moon Cycle 30d · 100 Days 100d) · stage-driven disclosure (apply `stageIndex` to dashboard/profile/rails) · `practice.verified` host/peer verification + device attestation/P2P mutual-confirm · realtime reward feedback via Broadcast · **Programs content depth + program-as-template "Add to Circle"** — ✅ **re-pointed 2026-08-17, not retired.** The markdown-framework version is gone, but [ADR-864](DECISIONS.md) rebuilt Programs on Channels and "add to Circle" is now the **Remix flow starting a Chapter** from the channel's blueprint (`lib/circles/remix.ts` sets `origin_template_id`). Depth work means more blueprints, not more markdown · community-library moderation + promote-to-tracked Journey · seasonal-Journey authoring surface + content (link to season + Pillar).
...
| `literal-radius` ~~2450~~ baseline **2440**, current **2288** | L | Only value-identical conversions were taken (`rounded-2xl`→`rounded-card` at 1rem, `rounded-lg`→`rounded-control` at 0.5rem). The rest are genuine resizes: `rounded-2xl`→`rounded-card` is 24px→16px elsewhere, `rounded-xl`/`3xl`/`md` have no 0.75/1.5rem role at all. DAWN §Phase 5 says spend this inside screen passes, never as its own wave — which is exactly where the unclaimed −152 came from |
