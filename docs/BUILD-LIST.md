...
- **Security:** ✅ **CSP (report-only)** + the `/api/csp-report` sink (next step: tighten + nonces → enforce) · RLS convergence Phase 2 (blocked on test harness) · rate-limit `check-handle`/`search-handles`/beta + webhook replay protection · admin audit log (partial: `platform_flag_events` · `studio_site_changes` · `reward_grants`).
