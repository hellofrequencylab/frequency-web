...
- **P5 — Other entity scopes to "full" + Account/"You" + global onto the contract.** DONE. The audit found the earlier phases had ALREADY converged the structure: the eight non-space scopes (global, circle, hub, nexus, event, practice, channel, journey) render from `ADMIN_MODULES` via `modulesForScopeKind`, and `appsForScope` (lib/apps/for-scope.ts) is the ONE seam resolving every scope — space via `spaceModuleManifest`, the rest (incl. global's personal `account.*` "You" set) via the scope-kind match — proven byte-for-byte by the `appsForScope` parity test. The retired `SPACE_SURFACES`/`spaceSurfacesFor`/`SpaceSurface` symbols have no live code path after P4 (only a historical comment + docs remain), so the duplication was already closed. The one substantive divergence left was COPY: five entity `*.settings` descriptions + `channel.settings` carried an em dash where SPACE's modules (and `journey.settings`) were already clean — normalized the six to the canon-compliant, SPACE-matching ". Edited in place." / ". Platform-curated, staff only." pattern. Modules with a full-page manager (`hub.layout`, `nexus.layout`, `journey.builder`) already carried a clear "manage from the console / full-page builder" description; no snapshots added (remit barred expensive new queries). The generic `ENTITY_SURFACES` /manage-console path is a distinct, already-single-sourced renderer (not a rail duplicate) and was left intact — deleting it would be a console behavior change for no convergence gain. ADR-548.

## Status — PROGRAM COMPLETE
- [x] P0 · [x] P1 (console) · [x] P1b (rail) · [x] P2 (inline bodies + snapshots) · [x] P3 (Module Manager, console-fed) · [x] P3b (rail on the manifest + honors order/hidden) · [x] P4 (`SPACE_SURFACES` retired) · [x] P5 (all scopes on the one contract; copy normalized to canon)
