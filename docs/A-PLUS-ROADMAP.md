...
- 🤖 ◧ Migrate raw `<img>` → `next/image` for LCP/bandwidth. **Done:** the 7 list **avatar** sites (avatar URLs validated to Supabase, `lib/profile-input.ts`) + the **circle cover** (uploaded to the `site-media` bucket via `uploadCircleCover` → Supabase host). **Remaining covers are HOST-GATED, not just a browser check:** before migrating channel/profile-header/journey/market covers, each needs its image URL confirmed to a `remotePatterns` host — `next/image` *throws at runtime* on an unconfigured host. Channel covers have **no upload action** (seed/admin-set, host unconfirmed); `profile-input.ts` validates only the avatar, not the header; journey/market images need the same check. So this is "confirm host → then `fill`," per surface. **Intentionally NOT migrated (wrong tool):** blob/data upload previews, the `/api/qr` image, map-marker overlays, the OG `ImageResponse` `<img>`s, the operator brand-mark logo, the topical placeholder host. (M)
