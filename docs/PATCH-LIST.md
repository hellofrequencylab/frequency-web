...
- **Stripe dual-webhook, one secret (P2, ops)** — two routes (`/api/stripe/webhook` + `/api/webhooks/stripe`) share one `STRIPE_WEBHOOK_SECRET`; Stripe issues a distinct secret per endpoint, so one route's events (subscription-cancel/past-due downgrades on one; refund capacity-release on the other) are dropped on whichever isn't the live endpoint. Initial grants are de-risked by success-redirect fallbacks; the webhook-only reconciliations are not. Needs the live Stripe dashboard config + a consolidate-vs-second-secret decision.
