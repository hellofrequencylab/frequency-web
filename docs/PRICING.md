...
| **Household / Circle bundle (P2)** | `lib/pricing/bundle.ts` + `bundleSellable()` + `lib/billing/bundle-checkout.ts`; config + `profiles.household_bundle_id` link in the migration. **Both halves shipped 2026-08-12:** the checkout stamps the owner, the seat roster and the purchased terms; the webhook branch `lib/billing/bundle-seats.ts` seats them through `apply_bundle_seating_atomic` (migration `20270225000000_household_bundle_seating.sql`, **written, NOT applied**). Reachable via `startBundleCheckout` (`app/(main)/settings/billing/actions.ts`); the seat-picker UI is the remaining piece. | `bundleSellable` = `billingLive()` AND `bundle_household_enabled` (OFF); checkout returns null while OFF, so no bundle subscription exists for the webhook branch to seat. |
