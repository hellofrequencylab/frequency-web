...
**Design principle: list-first (owner requirement, locked).** The **member list is the CRM's default view and is always one tap away.** This holds at **both** altitudes: the Space CRM (`/spaces/[slug]/crm`) lands on the familiar People roster, and the **platform Resonance CRM** (`/admin/crm`) lands on the **Members** roster, never the cockpit. The cockpit (verdict + health stats + worklist + lifecycle funnel + rising members + score trustworthiness) and the pipeline are **secondary** views, reached from a persistent, always-visible tab bar (`components/spaces/crm/crm-view-tabs.tsx` for the Space, `?view=people|pipeline|cockpit` with People default; `app/(main)/admin/crm/crm-view-tabs.tsx` for the platform, `?view=cockpit` with Members default). That same tab bar renders on a person detail and on a funnel drill, so an operator returns to the roster in one tap from anywhere. The cockpit is the smart layer the operator opts into; the list is the front door they live in.
...
### Altitude 1 - Platform Resonance CRM (`app/(main)/admin/crm/page.tsx`)

✅ **Shipped (platform surface, list-first).** Mirroring the Space CRM (#1303), the platform Resonance CRM is **list-first**: `/admin/crm` lands on the **Members** roster (the full scored roster via `listAllScoredMembers` / `listMembersByFilter({ kind: 'all' })` in `lib/dashboard/scores.ts`, lowest health first), so the scored members are visible up front even when the action panels are calm (the expected first-reading state, when `getWorklist` returns 0 sliding rows). A persistent tab bar (`app/(main)/admin/crm/crm-view-tabs.tsx`, Members default · Cockpit) keeps the list one tap from the cockpit and from any drill. The **Cockpit** is the secondary `?view=cockpit` tab: the computed verdict line, the four `StatCard`s, the who-needs-attention `Worklist`, the drillable lifecycle funnel, the rising-members pool, and the score-trustworthiness backtest. The standalone `/admin/crm/members` drill route now also defaults to the full roster (no `?tier=`/`?stage=` required); a tier band or lifecycle step still drills the same list down. Both the default view and the drill route render through the one shared roster component (`app/(main)/admin/crm/member-roster.tsx`) over the one shared reader, so there is no duplicated data access. Staff-gated (`requireAdmin('janitor')`); every read is fail-safe to a calm empty state. This is a UX restructure only: scoring and the worklist ranker are unchanged.
