...
`components/teaser-gate.tsx` is a reusable client wrapper:
...
- `TEASER_GATE_ENABLED` (**default `false`**) is the master switch. While the beta is
...
- `TEASER_MIN_ROLE = 'crew'` is the tier that gets full access (crew and above).
- `TEASER_PREVIEW_SECONDS = 30` is the peek length.
- `teaserAllowed({ role, hasAccess })` is `true` if the gate is off, the viewer is
