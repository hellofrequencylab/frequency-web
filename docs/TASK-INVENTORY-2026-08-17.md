...
> [`BUILD-BACKLOG.json`](BUILD-BACKLOG.json). Not the live plan.
...
| **F-1 · Settle EDITOR-ARCHITECTURE's two 🔴 sequencing questions on paper** | Settled on paper by [ADR-1005](DECISIONS.md), written up in [EDITOR-ARCHITECTURE §9](EDITOR-ARCHITECTURE.md). "E1 before E2 is circular" was never a cycle — E2 needs E1 and not the reverse; what read as circular was E1's GATE quantifying over the whole catalog, so it now quantifies over ADOPTED rows (target `ENTITY_BLOCKS`, 36) and completeness moves to E2. The three missing contract pieces re-filed by phase: field/UI metadata stays in E1 with a `fields` ⇄ `content` bijection, per-type limits move to E0, and the role floor splits (`minRole` on `BlockNode` at E0, `defaultRole` on `defineBlock` at E1) |
| **White-on-amber button text** | Decided by [ADR-1031](DECISIONS.md) (2026-08-13): white ships on the unchanged `#E2912F`. `--color-text-on-primary` is `#FFFFFF`, the amber never moved, and the shortfall is disclosed as a named frozen waiver in `scripts/check-contrast.mjs` rather than beaten. The letterpress finish that briefly made axe measure a stroke instead of the fill came back out under [ADR-1096](DECISIONS.md) (2026-08-21) — a finish is not mitigation |
