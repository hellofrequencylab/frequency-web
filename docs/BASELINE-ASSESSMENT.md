...
- ⏳ **Optional residual (owner-run):** the file still exists in ~271 stale **merged** branches (private repo). Low-severity; prune them from a dev machine (`git push origin --delete …`) to fully purge. It doubles as the long-owed merged-branch cleanup. The cloud env's git proxy can't delete branches.
