...
Status: **Phases 1 to 3 shipped.** Decision: [ADR-069](DECISIONS.md). Projects off the event
...
| **6 · Intelligence & Activation** | wide `interaction_events` capture → feature store → predictive traits → AI site-improvement loop → retroactive reward engine (track PI, [ADR-166](DECISIONS.md)) | ⏳ all 5 caps built (PI.1 to PI.5) |
