...
**Consequences.** Migration `supabase/migrations/20260801100000_menu_gate_columns.sql`. Types: `lib/menus/types.ts` (`ResolvedItem` += `staffDomain`/`staffLevel`; `ResolvedCategory` += `minAccess`/`staffDomain`/`staffLevel`/`icon`/`blurb`, all optional). Reader/actions: `lib/menus/read.ts` + `lib/menus/actions.ts` thread the new fields through (validators `isStaffDomain`/`isStaffLevel`, persisted in `seedMenuFromDefaults`). Renderer helper: `components/layout/menu-role.ts` (`canSeeMenuItem`/`canSeeMenuCategory`/`canSeeMenuEl`, not yet wired into any renderer this phase). Editor: new `components/admin/menu/gate-controls.tsx`, wired into `item-editor.tsx` + `menu-groups-editor.tsx`. `tsc` + `eslint` clean. Behavior is unchanged this phase (no renderer reads the staff axis yet). **Deferred to later phases:** merge `public_discover`+`public_explore` → `header`; rename `marketing_footer` → `footer`; new `user` container for the account dropdown; fold `admin_subheader` into `left` with a contextual sub-header (retiring `ADMIN_NAV`); the cross-container "menu selector" move action + dropdown. The admin **dashboard pages** (`ADMIN_GROUPS`) stay code-driven (out of scope), so nav and dashboards may drift — accepted for now.
