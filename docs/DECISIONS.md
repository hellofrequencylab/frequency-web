...
**Decision:**
- **One identity.** `display_name = 'Vera'`, callsign **@vera** (renamed from @moderation by the parallel workstream's 20260615400000, which also de-keyed the warn-DM lookup to `is_system` alone — no code keys on the handle anymore). Bio seeded from her assistant greeting. The same pass moved the newcomer's personal greeting into a **welcome notification** (lib/onboarding/welcome.ts now does both: the public system line AND the notification).
