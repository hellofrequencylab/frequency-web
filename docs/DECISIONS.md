...
## ADR-304: Rewards Economy v3 — the Season 1 clean rebuild (one classifier, two ledgers, validated creation)
...
**Retired / cut this rebuild.** Witnessed peer awards · secret awards ("Quiet Ones") · all circle-collaborative mechanics (Co-op Pulse, Co-op Synchrony, Carrier Wave, Circle Current) · the Practice Shelf (consistency / depth ladders) · Side Quests · the retroactive reward **rules engine** (the `reward_grants` **table stays** as the season-conversion + idempotency ledger) · the recruiter / entry-point reward **leaderboard** (core QR entry-point capture stays). **Founder's First Week onboarding is KEPT.**

**Season 1 clean start.** Beta reward data is wiped and S1 is reseeded: one active season + Quest; three Journeys (Mind / Body / Spirit) each capped by one Expression Challenge; the lean achievement set; the lean store catalog; an `earn_rules` registry holding the amounts/caps above.

**Consequences.** The economy reduces to one classifier, two ledgers, and a registry of amounts — every other surface (achievements, challenges, Journeys, creation) reads the same rule, so "what does this pay" has exactly one answer. The creation reward is now adoption-gated, which both throttles spam (no payout for an asset nobody uses) and rewards genuinely useful contribution. The wipe is a one-time event; after S1 the model is additive. Migration-level detail (the `earn_rules` registry shape, trigger definitions, the wipe/reseed migration) lands in [DATABASE.md](DATABASE.md) and the migration set as it is built; this ADR + [REWARDS-ECONOMY.md](REWARDS-ECONOMY.md) are the spec it follows. **Honor-system note (carried from ADR-287):** the Expression capstone Circle-vs-online choice stays self-attested. **Open follow-ups:** the exact `earn_rules` table columns and the Spark cap/frequency tuning are set during the build.
