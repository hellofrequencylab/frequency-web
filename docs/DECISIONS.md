...
**Consequences:** One nav surface instead of three; the operator never loses horizontal space to a rail. The chrome stays 100% declarative (no path-sniffing in `app-shell`). Reversible by restoring the two deleted components + the `railFor` line. **Follow-up sweeps (separate PRs):** retire the duplicate Marketing/CRM in-page sub-navs + the scattered in-page back-links (consolidate onto `PageHeading`'s `back` slot); templatize the remaining ~22 non-`AdminTemplate` pages; standardize hand-rolled buttons onto `components/ui/button.tsx`. Optionally derive `AdminTemplate`'s title/eyebrow from `pageLabelForPath`/`domainForPath` so titles can't drift from the menu labels.
