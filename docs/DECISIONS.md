...
## ADR-210: Surprises — variable-ratio, gems-only bonus on the daily practice loop

**Status:** Accepted · `lib/surprises.ts` (`rollSurprise`/`fireSurpriseForLog`), hooked in `lib/practices.ts` beside the journey/co-op reward firing, `lib/surprises.test.ts` — builds on ADR-200 (journey reward firing) and the `reward_grants` claim-then-pay idempotency (ADR/`lib/rewards/evaluate.ts`)
