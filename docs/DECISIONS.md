...
**Decision:**
- **One identity.** `display_name = 'Vera'`; the **@moderation handle is KEPT** as her callsign — the warn-DM flow keys on it (report-actions.ts) and handles are member-visible plumbing, not the name. Bio seeded from her assistant greeting.
