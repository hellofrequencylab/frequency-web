...
**Status:** Accepted · migration `supabase/migrations/20260614100000_stewardships.sql` (**apply pending** — applied to prod separately), `lib/core/stewardship.ts` (+ test), `lib/stewardships.ts`. Implements BUILD-LIST P1.4 / docs/ROLES.md §"System 1" + "Data model". Mirrors the P2.1 entitlement foundation→flip split (ADR for `membership_tier`).
...
## ADR-220: Store redemption trigger no longer touches lifetime_gems (double-charge fix)
