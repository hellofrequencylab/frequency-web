...
- **Metadata-only template catalog.** `lib/widgets/templates.ts` (`TEMPLATES` / `TemplateId` / `isTemplateId` / `templateMeta` / `slotIds` / `defaultSlotId` / `DEFAULT_TEMPLATE`) holds **no React** — exactly like the module catalog (`lib/widgets/modules.ts`) — so the editor, the save actions, and the pure resolver import it without pulling the grid components (those live in the renderer). **Adding a template = one entry here + one grid case in `page-modules.tsx`.**
...
**Status:** Part 1 Accepted + shipped (2026-06-16); Parts 2 + 3 PLANNED. — "Make the journey practices the 4 pillars. Mind, Body, Spirit and Expression. Add challenges as 'extra credit' above and beyond the journey that counts towards regular points. Somewhere in gamification we should have Side Quests: reward based, not counting towards the 4 pillars, a special badge for finishing one." Supersedes ADR-299's "Expression as a challenge-style slot."
...
2. **Extra-credit Challenges (planned).** Optional "above and beyond" tasks attached to a Journey that **pay regular Zaps** (`awardZaps`) on completion but are not part of the four practices. Likely a bonus block in the curriculum (or a Journey-linked `season_challenges` row); they challenge the individual and count toward regular points, not a separate Pillar tally.
3. **Side Quests (planned).** Self-directed, reward-only missions that grant a **special badge** and do **not** touch the Pillar Signature. Build on the existing badge engine (`achievements` + `user_achievements`, with `tier`/`icon`/`criteria`/`zaps_reward` and `grantReward`), surfaced as a browsable board (e.g. `/crew/side-quests`). A Side Quest = a badge-granting achievement flagged as a side quest; completion grants the badge + its reward, no `practice_logs`, no Pillar credit.
