...
## ADR-358: The Founders Round is a founding-member PRE-SALE (not a donation), collected through PayPal Standard Checkout

**Status:** Specified, not built (2026-06-22). Spec: [FOUNDERS-ROUND.md](FOUNDERS-ROUND.md). Will ship behind an env gate (`paypalEnabled()`), mirroring the dormant Stripe layer (`lib/billing/stripe.ts`, ADR P2.2). Migration written but applied to prod by the owner via the Supabase SQL Editor (never `supabase db push`). Distinct from the display-only Organization donation asks (`space_donation_asks`, `20260716000000`), which stay as-is.

**Context.** The owner wants to raise $10k to $25k in seed capital from a personal social following to fund creating the nonprofit (Frequency Foundation), the legal work, and the first events and operations: the pre-investor "community launches for the community" layer. The instinct was a GoFundMe-style donation drive deposited to a personal business PayPal. Research surfaced a hard blocker: Frequency is a **for-profit** today, and (1) PayPal's Campaign Fundraising terms prohibit offering any reward or perk in return for a contribution and require registered-nonprofit status to use the Donate / fundraising product; (2) for a for-profit, the money is taxable business income and backers get no deduction, so "donation" / "tax-deductible" is misleading; (3) the FTC enforces stated refund and use-of-funds promises (Chevalier 2015, iBackPack 2020). The owner also confirmed the platform's free/paid model: the whole app (Circles, practices, Journeys, events, earning Zaps/Gems) is free for everyone, and paid membership only unlocks "cashing in" the game (Vault Store spend, public rank badge), so founders are not buying access others lack.

**Decision.** Run the raise as a **founding-member pre-sale**: the GoFundMe feel (live tracker, milestones, backer feed, comments) with Kickstarter language and mechanics (back, pledge, tiers), where each tier sells a pre-paid membership plus founder recognition, which is a sale by definition and fits PayPal **Standard Checkout (Orders v2, `intent: CAPTURE`)**, funds settling immediately. Copy never says "donation" or "tax-deductible." Locked configuration from the owner Q&A: a straight 30-day public push; goal $10k to $25k; anchor perk = 1 year of the paid (`crew`) tier plus a lifetime founding rate-lock; a $25 / $60 / $150 / $500 ladder plus stackable add-ons (bonus Gems, gift a membership, an extra Space, time with the founder); tiered founder badges plus a public Founders Wall with opt-in anonymity; momentum via an early-bird limited tier, a live backer feed with comments, and stretch goals past the target (no match challenge); keep-what-you-raise, all sales final, perks delivered regardless; a public `/founders` page allowing guest or member backing plus an in-app banner; a use-of-funds breakdown with percentages. The build reuses existing seams: a new `fundraise_*` table family (RLS on, no client policies, service-role writes, untyped until `lib/database.types.ts` regen per ADR-246); a `PAYMENT.CAPTURE.COMPLETED` webhook as the tracker's source of truth with a `fundraise_webhook_events` idempotency table (clone of `stripe_webhook_events`); membership fulfillment by setting `membership_tier='crew'` plus a new `membership_paid_through` expiry on `profiles` (NEVER touching `community_role`, per ADR-207 / ADR-225), so Stripe later extends the same grant; and a "Founding Member" achievement badge (net-new `achievements` rows, like `founders-first-week`) plus a `member_tags` cohort tag.

**Consequences.** The raise is legal, honest, and on-brand: backers fund a launch and get recognition plus a head start, while the app stays free for everyone (which is the stronger pitch and passes the skeptic test). Pre-sale revenue is taxable business income and may carry state sales-tax obligations; the "all sales final" stance, the refund/disclosure copy, the collecting entity (for-profit now, Foundation later, tied to ADR-031 / ADR-037), and revenue recognition are flagged for counsel + a CPA before launch. PayPal API specifics and fees must be re-verified in a browser before building (PayPal blocks automated doc fetches). Deliberately excluded: PayPal's Donate button / IPN (deprecated and charity-gated), a page countdown timer (unreliable, sometimes negative), and a match challenge (no matcher). The display-only `space_donation_asks` is untouched; per-Space campaigns are a later phase via the nullable `fundraise_campaigns.space_id`. No copy carries em or en dashes.
