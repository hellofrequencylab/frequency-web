...
## ADR-390: Unified navigation — four standardized, role-gated menu containers

**Status:** Accepted (2026-06-24). Standardizes ALL site navigation into **four editable containers** — **Header**, **Left**, **Footer**, **User** — built on the existing DB-backed menu system (`lib/menus/`, from migration `20260721000000_menu_system`). Every group/page/link is assignable to any container, visibility is purely role-based (a basic member sees a little; complexity scales up to Janitor, who sees everything), and the admin nav becomes the high-role end of the **Left** container rather than a separate surface. Shipped in **phases**; each is additive and fail-safe. This entry lands **Phase 1: the data-model gate + editor**; later phases merge the surfaces, add the User container, fold admin into Left with a contextual sub-header, and add a cross-container move control.

**Context.** Navigation was defined in five ad-hoc menu "surfaces" (`public_discover`, `public_explore`, `admin_subheader`, `left_rail`, `marketing_footer`) plus a hardcoded account dropdown (`components/layout/user-menu.tsx`), with the admin nav additionally hand-maintained in three code modules (`lib/admin/nav.ts` `ADMIN_NAV`, `lib/nav-areas.ts` `NAV_AREAS`, `app/(main)/admin/sections.ts` `ADMIN_GROUPS`) that had drifted. The owner directive: collapse this into four standardized containers an operator can fully reshape, with any item movable to any container and role-based visibility as the spine. The load-bearing risk: admin pages gate on TWO axes — a community/role floor PLUS a staff capability domain (`staffDomain`/`staffLevel`, ADR-127/ADR-208) — but the menu tables could only express `min_access` + per-role `role_modes`. The editor (`item-editor.tsx`) even hard-pinned `min_access='visitor'` on every save, so a staff gate could not be expressed at all. Moving a staff-gated page into a container would have silently lost its gate.

**Decision.**
1. **Carry the full two-axis gate on menu data.** Migration `20260801100000_menu_gate_columns.sql` adds `staff_domain` + `staff_level` to `menu_items`, and `min_access` + `staff_domain` + `staff_level` + `icon` + `blurb` to `menu_categories`. `staff_level` is CHECK-constrained to the `Access` ladder (`none|read|write`); `staff_domain` is free-text validated in the action layer against the `StaffDomain` set (so a future domain needs no migration). All nullable/defaulted — additive and inert until renderers read it. No type regen needed (menu tables use the untyped `menuDb()` handle, ADR-246).
2. **One union gate, applied identically by every renderer.** New helpers in `components/layout/menu-role.ts` (`canSeeMenuItem` / `canSeeMenuCategory` / `canSeeMenuEl`) decide visibility as the UNION of (a) the menu-access path (`effectiveMode` — `role_modes` + the `min_access` floor) and (b) the staff axis (`staffCan(staffRole, el.staffDomain, level)`, default level `write` for a leaf, `read` for a section). This mirrors `lib/admin/sections.ts` `canUseLink`/`canSeeGroup` exactly, so folding admin into a container preserves access. Crucially the helper takes the FULL viewer (community token + staff role), not the collapsed `MenuAccess` token, so a functional operator (e.g. a Marketer whose community token is just `member`) still sees their domains. **Never weakens a gate:** a viewer who fails both axes is hidden, and pages still re-gate server-side (`requireAdmin`), so a menu mistake can never grant page access.
3. **Restore the gate in the editor.** `item-editor.tsx` stops pinning `min_access='visitor'` and renders a shared `GateControls` (min access + staff domain/level); `menu-groups-editor.tsx` `CategoryHeader` renders the same for sections. The per-role `role_modes` matrix remains as the finer override layer on top of the floor.
4. **Defaults carry the gate.** `lib/menus/defaults.ts` (`item`/`category` helpers + `adminMenu`) propagate each admin section's `min`/`staffDomain` onto its category AND items, so seeding from defaults reproduces today's gate instead of dropping it.

**Rationale.** The gate-fidelity work had to come first and on its own: it is the one change that is security-load-bearing, and getting it in (with the renderers still using their existing gates) means every later phase — merging surfaces, the contextual sub-header, the cross-container move — can move a staff-gated item around safely. Unioning the two axes (rather than collapsing staff into the single token) is what keeps department operators seeing their surfaces while never granting anything the page itself would refuse.

**Consequences.** Migration `supabase/migrations/20260801100000_menu_gate_columns.sql`. Types: `lib/menus/types.ts` (`ResolvedItem` += `staffDomain`/`staffLevel`; `ResolvedCategory` += `minAccess`/`staffDomain`/`staffLevel`/`icon`/`blurb`, all optional). Reader/actions: `lib/menus/read.ts` + `lib/menus/actions.ts` thread the new fields through (validators `isStaffDomain`/`isStaffLevel`, persisted in `seedMenuFromDefaults`). Renderer helper: `components/layout/menu-role.ts` (`canSeeMenuItem`/`canSeeMenuCategory`/`canSeeMenuEl`). Editor: new `components/admin/menu/gate-controls.tsx`, wired into `item-editor.tsx` + `menu-groups-editor.tsx`. `tsc` + `eslint` clean.

**Admin header (the fifth surface, ADR-390 follow-up).** The owner clarified that the admin sub-header should NOT be removed — it should come back as its own editable mega surface, **`admin_header`**, that is CONTEXTUAL: clicking an admin entry in the `left` rail lands on that section's dashboard, and the `admin_header` renders only THAT section's sub-pages as a mega bar above the content (with the admin / Vera `AdminSearchBar`). Built from `ADMIN_NAV` (section → groups → links) via `adminHeaderMenu()`; the shell resolves the active section by longest href-prefix (`adminSectionForPath`) and re-roots the menu to its children so `MegaBar` (`triggerLevel='category'`) draws the section's sub-pages as tabs/dropdowns. It's a full member of the menu system (assignable slots, the gate, the move dropdown) — `GRID_SURFACES` includes it. Migration `20260801130000_menu_admin_header.sql` widens the `surface_key` CHECK to the five keys. So the system is **four primary containers + the contextual admin_header**.

**Primary model — the four containers (shipped, owner directive "tear it apart and make it clean").** The five ad-hoc surfaces collapse into the four primary containers, named for what they are: **`header`** (the mega-menu for general site nav — `public_discover` + `public_explore` become its two top-level dropdown categories, rendered by `MegaBar` at `triggerLevel='category'`), **`left`** (the in-app rail / "user menu in the feed" — admin simply lives here as high-role sections via `NAV_AREAS`, role-gated by the two-axis `canSeeMenuItem`; there is NO separate admin sub-header any more — it was deleted, and `/admin` pages navigate via their own in-content chrome), **`footer`** (`marketing_footer` renamed), and **`profile`** (the account dropdown — both the public `UserMenu` and the in-app `AccountDropdown` render their editable link list from this surface, framed by fixed Profile / Invite / Report-a-bug / theme / Sign-out chrome). Migration `20260801120000_menu_four_containers.sql` renames `left_rail`→`left` and `marketing_footer`→`footer` in place (preserving customization) and drops the structurally-reshaped `public_*` / `admin_subheader` rows (they re-render from the rich code defaults). `MenuSurfaceKey`, `MENU_SURFACES`, `SURFACE_KEYS`, `GRID_SURFACES` (now just `['header']`), and `defaults.ts` (four builders: `headerMenu`/`leftMenu`/`footerMenu`/`profileMenu`) all reduce to the four. Every header/footer/account/rail consumer (`primary-nav`, `marketing-header`, `site-header`, `app-shell`, `user-menu`, the marketing/help/main/root layouts) points at the new surfaces. **Cross-container move** ("put any page anywhere"): `moveItem` / `moveCategory` server actions (re-stamping `menu_id` across the whole subtree) + a `MenuMoveField` dropdown wired into the link editor and group header. `lib/admin/nav.ts` (`ADMIN_NAV`) is no longer used by the menu system (Phase-6 deletion pending a grep). The admin **dashboard pages** (`ADMIN_GROUPS`) stay code-driven (out of scope), so nav and dashboards may drift — accepted. `tsc` + `eslint` + `next build` all green.
