...
**Status:** Accepted. Migrations `20260720000000_practice_mindless_mode.sql`, `20260720000100_practice_log_completion.sql`, `20260720000200_practice_timer_data_fix.sql` (apply pending; the integrator regenerates `lib/database.types.ts` after). Code: `lib/practices.ts` (`logPractice`, `getPartialPracticesToday`, `pillarTimerDefault`, `MINDLESS_MODES`, the `Practice` + `LogPracticeResult` additions, `normalizePractice`, `practiceFullReward`), `app/(main)/on-air/actions.ts` (`completeSession`), `app/(main)/practices/actions.ts` (`logPracticeAction`), `lib/on-air.ts` (`RevealPayload` additive fields). Realizes the practice-timer redesign data model + completion economy.
