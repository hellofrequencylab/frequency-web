...
| `SCAN-638` | Six unindexed FKs + two initplans | Closed on main (#2764) | Stay done. |
| `LIVE-242` | Fold Hubs/Nexuses | Done (#2734) | Stay done. Do not start from this scan. |
| `SCAN-640` | Backup table, no PK | Closed on main (#2766) | Stay done. |
| `SCAN-641` | Two public headers | Closed on main | Stay done. Header only. |

**New findings that survived refute.**

| Surface | What crawlers are told | What the route actually is | Row |
|---|---|---|---|
| Spotlight | sitemap `getSpotlightRoutes` + Person JSON-LD + self-canonical | `revalidate = 3600` + `generateStaticParams` (closed #2741) | `SCAN-642` done |
| Event + listings | sitemap + `revalidate` | now under `app/(public)/` (no cookies/headers) | `SCAN-643` done |
| Space profiles / Shows | sitemap `/spaces/<slug>` | Space root layout still calls `getMyProfileId` | `SCAN-644` |
...
| med | **Claim tokens are plaintext at rest.** ADR-907's hashed system was retired *unimplemented* ([ADR-1108](DECISIONS.md), 2026-08-24): `lib/claims/tokens.ts` is deleted and `public.claim_tokens` is dropped, because it had 0 rows and 0 importers a year on. The **anon** half is closed by the three `2027012[789]` column revokes (`has_column_privilege` = false on all four tables). What is still open is the other half: all four flows write a **plaintext, non-expiring** token to an entity column, and **37 unredeemed tokens are live** (`spaces` 9, `events` 27, `market_listings` 1), readable by anyone with service-role or direct DB access. | Owner's call. Either accept the at-rest exposure explicitly, or reopen [ADR-1108](DECISIONS.md) and rebuild the hashed system — a fifth claimable entity is the trigger that makes it pay for itself. Reasoning + measurements in [`CLAIM-LINKS.md`](CLAIM-LINKS.md) §4. |
...
- ~~Circle handoff has no way to see or cancel a pending offer, so an unanswered offer blocks the Circle permanently (`app/(main)/spaces/[slug]/manage/circles/actions.ts:161`).~~ **Re-verified fixed 2026-08-20:** `cancelCircleOffer` exists (`lib/circles/handoff`) and `cancelSpaceCircleOfferAction` exposes it (`actions.ts:203-210`, "Take back a pending handoff before it is answered").
...
- The sync engine still inserts-only: build the non-destructive "re-check for new pages" action,
  ...
  **Re-verified 2026-08-20, 2 of 3 done:** `syncMenuFromDefaults` is the non-destructive re-check
  ...
  manager (`components/admin/menu/` has none). **Re-tested 2026-08-25 and PROMOTED OUT of the HYG-010
  sweep into its own row, `LIVE-111`:** it needs a derivation over (resolved menu, `defaultMenu`,
  `synced_default_keys`) plus a prop threaded into a 900-line client editor, which is a feature and
  not a badge. `LIVE-107` shipped the WEEKLY half (`scripts/maintenance/menu-drift.mjs`); the
  operator standing in the editor still sees nothing, and the mislabelled row LIVE-107 fixed was
  created in that editor.
...
- **Performance** (docs/PAGE-FRAMEWORK §5): authed `(main)/layout.tsx` serial-await tail →
  ...
  **Re-verified 2026-08-20, all but one part landed:** marketing layout bans `cookies()`/`getUser()`
  ...
  (`lib/help/content.ts:28-35`). ~~Still open: raw `<img>` — 113 sites remain.~~ **🔴 Re-measured
  2026-08-25 and RETRACTED:** 134 `<img` lines, and **zero take a static local asset** — every `src`
...
- **a11y**: ~~missing `error.tsx`/`not-found.tsx` for some route groups~~ (9 top-level route groups
  ...
  **Re-verified 2026-08-20:** 3 of 4 sub-items closed as noted inline. **The fourth closed 2026-08-25
  ...
  on a failed delete; now `ActionResult`, both call sites read it), `InlineCover.onRemove` (the cover
  actions THROW, and a throw inside a transition callback is an unhandled rejection, so a refused
  delete cleared the preview anyway), and the event gallery's `handleGalleryChange` (a rejected
  reorder left the new order on screen and the old one in the database).
