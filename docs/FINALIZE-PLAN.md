...
| 6.7 | **Remaining ratchet tails** | M | Re-derived 2026-08-12 (`node scripts/check-adoption.mjs`, baseline / current): `raw-input` **119 / 118** (the borderless variant this row asked for **shipped** — `components/ui/field.tsx:105` `FieldVariant = 'boxed' \| 'seamless'`, so what is left is call-site work), `literal-display-type` **96 / 96**, `raw-button-bg` **524 / 515** (still replace the proximity-window pattern with the opening-tag form under a new basis fingerprint), `literal-radius` **2,440 / 2,298** (**spend inside screen passes, never as its own wave**). The 186 and 526 this row carried were quoted from a column, not a run. |
...
> classes, not 14, and `literal-radius` is 2,298, `raw-button-bg` 515, `raw-input` 118,
