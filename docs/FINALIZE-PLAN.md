...
| # | Item | Size | Detail |
| :--- | :--- | :---: | :--- |
| 6.1 | **Label association sweep** | M | **103 of 229** `<Label>` uses carry no `htmlFor`. Mechanical: wrap in the existing `Field` primitive, or thread `htmlFor`/`id` where the control is real and `role="group"` + `aria-labelledby` where it is a button group. Highest volume: `circle-builder.tsx` (12), `movement-session.tsx` (9), `events/drafts/[id]/editor.tsx` (9), the six onboarding `*-render.tsx`. |
| 6.2 | **Icon-button accessible names** | S | **48 of 82** `<IconButton>` uses have neither `aria-label` nor `label`. |
