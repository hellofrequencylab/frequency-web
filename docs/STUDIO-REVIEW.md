...
| 7 | **`/studio` home KPIs + time axis (deltas + sparklines) + retention heatmap** | Studio | S to M | Turns correct-but-static metrics into a read on growth vs decay |
...
| **Circle "practice of the week" hero + surface the practice prompt beyond the feed** | Practices | S to M | Social-accountability multiplier; more log surfaces |
...
| **Beta segmentation field** (interest/neighborhood → `contacts.meta`) + **testimonials/faces block** | Marketing | S to M | Segmented invites; human proof = trust |
