...
- **`topical_channels`** = the topical **Channel** (NAMING.md; never Interests or Topics), sorting
...
**Naming:** **Pillar** = a `pillars` row (Mind/Body/Spirit/Expression). **Channel** = a
`topical_channel` row. Since 2026-09-15 that is the ONLY channel table — the hierarchy-v2
`channels` pair was dropped, empty ([ADR-1349](DECISIONS.md)).
