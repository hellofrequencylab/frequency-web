...
| Gap | Why it blocks |
|---|---|
| ✅ Streaming on the Vera chat | CLOSED 2026-09-08 ([ADR-1287](DECISIONS.md), PROG-E8 increment). This row said "zero `.stream(` hits". `runToolLoop` streams through the SDK when handed `onText`; `POST /api/vera/turn` answers NDJSON and both Vera surfaces render the arriving reply in an `aria-hidden` draft bubble, appending it whole to the `role="log"` transcript. Usage still comes from each round's final message. Generation paths that need a whole document (Sparks, reviews, composers) deliberately keep the blocking call |
| 🔴 No per-Space retrieval | Vera has only `name / type / brandName / about`. `lib/ai/space-copilot.ts:18-22` names this deferred |
| ✅ zod at the reply boundary in `lib/ai/**` | CLOSED 2026-09-08 ([ADR-1287](DECISIONS.md)). This row said "no zod in any AI path". `lib/ai/schema.ts` (`parseModelJson` + a zod schema per reply contract) now reads every `JSON.parse`-over-model-output site in `lib/ai/**` (six), each fail-closed on its existing surface and each tested against a malformed reply. ⏳ What it does NOT yet cover: the hand-written `coerce*` readers over `tool_use` inputs in the Sparks and the importer / seeder / CRM extractors outside `lib/ai/`; that is the "does not scale past ~30 sites" half and stays with E8's composer generalisation |
| ✅ Vera's loop caches its stable prefix | CLOSED 2026-09-08 ([ADR-1287](DECISIONS.md)). This row said the loop "re-sends voice primer + persona + tool schemas every round". `SystemPrompt` is now `{ stable, volatile }`; `buildRequestPrefix` marks the stable block with `cache_control` after the tools and before the volatile block, built once per turn and reused byte-identical across rounds (`complete.test.ts`, `agent-claude.test.ts`). ⚠️ The marker is a request: on the default `haiku` tier the prefix (~3,600 to 4,000 tokens) sits at or under that model's 4,096-token cache floor, so the entry may not be written until the tier moves or the prompt is measured with `count_tokens`. The loop's returned usage carries `cacheCreationInputTokens` / `cacheReadInputTokens` and the ledger prices them; surfacing the two counts is on the row |
...
**49 of 606 migrations carry an explicit `-- ROLLBACK:` block with hand-reversal SQL**, and that *is*
...
| **E1** | Block contract. One registry, Zod schemas, up/down migrations, **`resolveAt`**, the binding layer. 🔴 **The five gates are NOT live** (corrected 2026-08-11). None of `check:doc-safety`, `check:blocks`, `check:surface-binding`, `check:loom-integrity` or `check:email-blocks` exists as a script, a `package.json` entry, or a member of CI's guard array. They land in E0 ([ADR-977](DECISIONS.md) D-10). This row previously read "already live", which conflated "would pass if written" with "is written" — a reader scheduling E1 would assume five gates guard the tree and find zero. What E1 does is turn them from green-on-today's-tree to green-on-the-new-one, ONCE E0 has written them | **L** | Every registry row resolves to a renderer for every declared surface |
...
| ✅ ~~**E1 before E2 is circular**~~ — **SETTLED, and the premise was wrong** ([ADR-1005](DECISIONS.md)). It is not a cycle: E2 needs E1 (consolidation needs a target to consolidate INTO), and E1 does not need E2 (nothing in `defineBlock`, the Zod `content`, the migrations, `resolveAt` or the binding layer requires the row count to be final). One directed edge. What read as circular is E1's GATE — "every registry row resolves" is a completeness claim over the catalog, and catalog membership is exactly what E2 decides. **Restated:** E1's gate quantifies over ADOPTED rows, plus `check:blocks` fails any block reachable from a live palette that is not adopted; completeness moves to E2. **E1's adoption target is `ENTITY_BLOCKS` (36 rows)** — the only system spanning all three `kinds` and both render targets, it already carries field metadata, and `check:email-blocks`' 14 ⇄ 14 bijection starts green on it. `blocks-adopted` enters §7.3 at **36 / 304**. Historical row: | §3's own worked example — `frq/offerings`, `kinds: ['member','space']` — is an **E2 consolidation artifact** whose member legality is an **E3** widening. So the doc illustrates the E1 contract with an object that only exists after E2 *and* E3. Resolve explicitly: either **E1 = contract + gates + two pilot blocks** (and its gate "every registry row resolves" is then unachievable, so restate it), or **E1 = contract + port all 304** (and E1 is XL and contains most of E2). Do not leave it ambiguous |
| ⚠️ **The contract is missing three things** — **SETTLED, and RE-FILED** ([ADR-1005](DECISIONS.md)). All three are real; two were filed a phase too late. **(a) Field/UI metadata stays in E1**, with a `fields` ⇄ `content` BIJECTION in `check:blocks` — the type alone does not stop a second schema. **(b) Per-type limits move to E0**: task 10 deletes `rows-ops.ts` `normalize()`'s global dedupe, which is the only per-type limit the system has, so the slot is a prerequisite of that deletion. "Cannot be retrofitted" is overstated for this one; "cannot be deferred past E0" is exact. **(c) The `CommunityRole` floor SPLITS** — `minRole?: ModuleRole` on `BlockNode` at **E0** (it is persisted tenant data across 36 `page_settings.layout` rows that E2's migration rewrites; no slot means a host-only module goes public with no record of which nodes were gated, which §2.1 already calls a regression), and `defaultRole` on `defineBlock` at **E1**. Vocabulary is `ModuleRole`, NOT `ElementRole` — §5 says copy `element_settings` and §9 says carry the `CommunityRole` floor, and those are different ladders. Only `ModuleRole` has persisted rows. Historical row: | (a) **Field/UI metadata** — `content: z.object({…})` cannot express `label`, `placeholder`, `options`, `defaultValue`, `upload`, `pickerBlock`, which `FieldDef` carries today and **E5's gate depends on**; (b) **per-type limits** — E0 introduces `block-limits.ts` and `defineBlock` has no slot for it, so the policy immediately lives in two places; (c) **the `CommunityRole` floor** that only `page_settings.layout` carries, which Tier B needs. Add all three in E1 or they become a second schema later |
