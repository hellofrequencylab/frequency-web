...
> Product law: [`CORE-MODEL.md`](CORE-MODEL.md). Names: [`NAMING.md`](NAMING.md).
> This file is the 2026-06-28 hardening spec (ADR-439). Do not open it for "what to build next."
...
| H0-4 | **Error monitoring** | Stand up Sentry (or equivalent) for the Next.js app: server actions, RSC, and client. Tag by route and by `entity`/`space_id`. Today there is no centralized error capture. | ✅ `@sentry/nextjs` wired the Next-16 way (instrumentation hooks + `sentry.{server,edge}.config.ts` + `instrumentation-client.ts` + `withSentryConfig`); init gated on `NEXT_PUBLIC_SENTRY_DSN`/`SENTRY_DSN` (no DSN = inert, build still passes). Tagging helpers (`route`/`entity`/`space_id`) in `lib/observability/tags.ts`; `global-error.tsx` captures. Owner must set the DSN (+ optional source-map upload token) in Vercel to activate. |
| H0-5 | **Cron observability** | Instrument all 18 `vercel.json` cron routes with success/failure heartbeats + alerting (e.g. a dead-man's-switch per job). A silently dead `process-queue`, `weekly-digest`, or `season-go-live` is exactly the future problem to prevent. | ✅ Shared wrapper `withCronHeartbeat()` (`lib/observability/cron-heartbeat.ts`) wraps all 18 handlers: success → POST monitor URL, failure → Sentry capture + `/fail` ping + rethrow. Env-gated (`CRON_HEARTBEAT_BASE_URL` or per-job `CRON_HEARTBEAT_URL_<JOB>`); no env = no-op, never blocks a run. Vitest at `lib/observability/cron-heartbeat.test.ts`. Owner must create one monitor per job and set the URL(s) in Vercel. |
| H0-6 | **Performance baseline** | Capture p50/p95 latency + query plans for the hot read paths (feed, circle detail, people directory, practice log write, events catalog). These plans are the "before" for H3. | 🟡 Methodology + capture tables + per-path EXPLAIN SQL shipped in [OBSERVABILITY-BASELINES.md](OBSERVABILITY-BASELINES.md) §2; harness `scripts/perf-baseline.mjs` (documents the plan, prints the EXPLAIN SQL, samples p50/p95/p99 when `PERF_BASELINE_BASE_URL` set, safe no-op otherwise). **Owner action:** run against prod + paste numbers/plans into §2b/§2c. |
| H0-7 | **Cost baseline** | Snapshot current spend by vendor (Supabase, Vercel, Anthropic, Resend, Upstash) and per-1k-members unit economics, so scale decisions in H3 are cost-aware. | 🟡 Per-vendor + unit-economics tables in [OBSERVABILITY-BASELINES.md](OBSERVABILITY-BASELINES.md) §3; harness `scripts/cost-baseline.mjs` computes per-1k-members costs from `*_MONTHLY_SPEND_USD` + `BASELINE_ACTIVE_MEMBERS` env (template-only no-op unset; reads no secrets). **Owner action:** copy each vendor bill into §3a/§3b. |
| H0-8 | **Define SLOs** | Write target SLOs (uptime, p95 latency, error rate, queue lag, cron freshness) into this doc so later phases have a bar to hold. | ✅ Written with rationale in [OBSERVABILITY-BASELINES.md](OBSERVABILITY-BASELINES.md) §4: uptime **99.9%**, read p95 **<800ms** / write p95 **<1000ms**, error rate **<0.5%**, queue lag **<10min**, cron freshness **100% of jobs fresh** (per-job windows, paged via the H0-5 heartbeat). |
...
**H0 design backlog (NOT mechanical — do not auto-apply):** PostGIS/`vector` relocation out of `public` (carries the `spatial_ref_sys` ERROR); per-function `REVOKE EXECUTE FROM anon/authenticated` triage on SECURITY DEFINER RPCs (`adjust_ticket_sold`, `reset_season`, `recompute_community_level` are candidates); RLS policies for the 69 deny-by-default tables as features go live; the perf debt (unused-index prune, permissive-policy consolidation, initplan wrapping). Remaining H0 items: H0-3 type regen. (H0-4 Sentry + H0-5 cron heartbeats shipped — app/config wiring, env-gated, owner sets DSN + heartbeat monitor URLs in Vercel to activate. **H0-6/7/8 observability baselines shipped 2026-06-29** — [OBSERVABILITY-BASELINES.md](OBSERVABILITY-BASELINES.md) holds the perf-baseline methodology + capture tables + per-path EXPLAIN SQL, the cost-baseline per-vendor + per-1k-member structure, and the written SLO targets; `scripts/perf-baseline.mjs` + `scripts/cost-baseline.mjs` collect them, both env-gated + safe no-ops; owner runs them against prod and pastes the numbers/plans in to close H0-6/7.)
...
| ID | Task | Detail | State |
|---|---|---|---|
| H2-1 | **Finish RLS convergence (ADR-042/056)** | Migrate remaining high-traffic read/write paths from admin-client to RLS + `SECURITY DEFINER` RPCs, surface by surface, each with policy tests. Done: notifications, friendships, feed, feed-detail, messages, rooms, **profile-meta self-writes** (5 spotlight actions → session client), **store_items admin writes** (create/update/delete/toggle → session client; added the missing host+ DELETE policy). Enumerate and close the rest (CRM, commerce, spaces reads where appropriate). **`resonance_consent` deferred:** its self-writes are upserts and the `ON CONFLICT` arm doesn't satisfy a cross-table self-policy under RLS — needs a SELECT policy + non-upsert or `SECURITY DEFINER` RPC. | ⏳ |
