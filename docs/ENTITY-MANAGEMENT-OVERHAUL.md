...
| **Per-entity owner management** — circle host, hub guide, nexus mentor | ⚠️ ~40% | The real gap. Owners get only page "edit mode" + a light sidebar. **No Layer-2 management suite.** ~~Hub/Nexus edit mode is not even wired.~~ ✅ **Hub/Nexus in-page edit mode now wired** (EM1-4): both detail pages render the capability-gated Edit + Manage entries, opening the same SettingsDrawer Basics module circles/events use. |
...
| **Consistency across entity types** | 🔴 ~50% | Spaces = bespoke 7-tab; circles = edit mode; hubs/nexuses = edit mode (now wired, EM1-4); events = edit mode. No shared pattern. |
...
| **EM1-3** | **Harmonize Spaces** onto the framework — wrap/retire the bespoke 7-tab so spaces use the same spine + module registry; no feature loss. **✅ shipped:** the unified `/spaces/<slug>/manage` console (#1293) serves `practitioner` + `organization`, and the legacy `/spaces/<slug>/settings` index now REDIRECTS those two types to it (after the same `resolveSpaceManageAccess` gate, so a non-manager still 404s and the route never leaks). Every settings sub-page stays in place as the console's section targets; the other types (business / event_space / lab / partner) have no console yet so they keep the legacy hub unchanged. One management-entry rule lives in `spaceManageHref()` (ADR-453). **Completed in EM2-3:** `business`, `event_space`, `lab`, and `partner` now have console spines too, so every provisionable type except coaching is managed through the one console; the legacy hub redirects them all via `isConsoleSpaceType()`. | L2 + L3 |
| **EM1-4** | **Wire Hub/Nexus edit mode** into the PageAdminDock; bring them to circle/event parity. **✅ shipped:** the registry already declared `hub.settings` / `nexus.settings` (gated `hub.manage` / `nexus.manage`) and the SettingsDrawer already resolved both scope kinds — the gap was the on-page *entry*. Each detail page now renders, gated on its manage capability, an **Edit** button (opens the shared SettingsDrawer → Basics module, the same `open-settings` seam as circle/event) plus a **Manage** link to the existing `/{hub,nexus}/[slug]/manage` console. Every settings action re-checks the capability server-side (admin client bypasses RLS, so the action is the authority). No schema change. | L3 |
...
| **EM2-3** | **Space completion (non-money)** — seed content on create (ENTITY-SPACES Epic 1.10), per-tab module editor, Lab/Partner deep features, advanced availability, multi-cohort coaching, event capacity/waitlist. **🟡 "all Space profiles" slice shipped:** the unified `/spaces/<slug>/manage` console now serves EVERY provisionable type except coaching (added `business`, `event_space`, `lab`, `partner` to `SPACE_SURFACES` + `CONSOLE_SPACE_TYPES`, gated by each type's existing per-Space functions — business → CRM/email/memberships, event_space → tickets/check-in, lab/partner → the universal Members/QR/Insights/Billing per ADR-341). Each new surface WRAPS the existing settings sub-page (memberships/tickets/checkin/…); nothing rebuilt, no schema change. The legacy `/settings` hub now redirects all console types to `/manage` via `isConsoleSpaceType`. Remaining EM2-3 (seed content, per-tab editor, deep features, advanced availability/waitlist) is follow-on. | L2 |
...
2. **Spaces harmonization depth** — fully retire the 7-tab vs. keep it as a thin compatibility
   ...
   the console serves the legacy `/settings` index becomes a thin REDIRECT to `/manage` (retired as a
   destination, not as a route file); for the types the console does not serve it stays the working
   hub. As of EM2-3 ("all Space profiles") the console serves every provisionable type except
   coaching, so only coaching (and root) still land on the legacy hub. The settings SUB-pages are
   kept everywhere as the console's section targets, so no feature is lost.
