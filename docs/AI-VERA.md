...
Status: **Phases A to D shipped & live** (kernel, help-RAG voice, member memory, onboarding
...
> *Soft:* "You don't have to have it figured out. Most folks here didn't either. Tell me one thing you're actually hoping for, and we'll start there."
...
| `suggest_circle` / `suggest_practice` | Surface 1 to 2 best-fit options (semantic match, §9) | read-only |
...
1. **"Find your people" semantic matching.** Embed a member's stated interests/goals (reuse
   pgvector) and match to the 1 to 2 circles/practices/people most likely to land. Kills the
...
Phases A and B are shared with the support-system / AI-search initiative.
