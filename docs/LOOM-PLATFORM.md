...
> composition of typed primitives and existing schemas, **never arbitrary JavaScript**), amending
> [ADR-501](DECISIONS.md) on authority; and `app_instances` **gets its writers in E0**. The block half
> of this plan is specced in [`EDITOR-ARCHITECTURE.md`](EDITOR-ARCHITECTURE.md) §5.
>
> **⚠️ "Loom" means two things in this doc set.** Here it is the **App catalogue and control plane**.
> [`LOOM-EVERYWHERE-PLAN.md`](LOOM-EVERYWHERE-PLAN.md) uses it for the **image picker**
> (`components/loom/`, one file). Say which one you mean.
>
...
| **3 · Instances** | per-surface placement + per-placement override | **`page_settings.layout`** (page) · **`app_instances`** (editor/rail — schema, indexes and full RLS quad exist; 🔴 **zero writers, zero readers**, given writers in E0) · ~~`library_usages`~~ 🔴 **dropped** | 📐 designed | an instance references `(app_id, version)` + an optional `config_override` |
