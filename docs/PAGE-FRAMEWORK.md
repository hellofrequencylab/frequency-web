...
|---|---|---|---|
| **Stream** | `StreamTemplate` | a flow of items: Feed, Broadcast, a circle discussion | `eyebrow·title·description·action·composer`, or `hero` when the stream opens on a header BAND instead of a sentence (Around You opens on the map, ADR-1032). The two are a union: in `hero` mode the band owns the `<h1>`, so the heading props are typed `never`. |
