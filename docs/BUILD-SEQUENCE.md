...
> Status lives in [`BUILD-BACKLOG.json`](BUILD-BACKLOG.json).
...
| **P4.8** | **Admin spaces health dashboard** — `/admin/spaces` grouped by health status (At risk -> Needs attention -> Dormant -> Healthy), pure `spaceHealth()` + batched no-N+1 signals, tenancy tools preserved | ADR-473 | none | ✅ merged |
| **P4.9** | **Operator-identity context switcher + "Spaces you run" hub** — framing-only context cookie, account-chip switcher + identity badge, `/spaces/operating` hub | ADR-474 | none | ✅ merged |
| **P5** | **Tier × Mode re-strategization (site-wide)** — four first-class tiers (Pro/Business/Non-profit/Organization), add-ons folded into depth (AI metered), curated Modes per tier; ripples through plans/entitlement depth, Stripe catalog, pricing page, per-Mode marketing, create wizard, admin pricing console, copy | PRICING-LADDER-PLAN §1b · SPACE-MODES-PLAN §2d · ADR-472 | additive (plan values) | 🔨 in progress |
| **P5.1** | **Tier foundation** — `plans.ts` four tiers + AI sole metered add-on + every consumer | ADR-475 | none | ✅ merged |
| **P5.2** | **Four public-page layout templates** — Book/Schedule/Storefront/Hub, variant-aware blueprint bridge | [SPACE-MODE-TEMPLATES](SPACE-MODE-TEMPLATES.md) · ADR-476 | none | ✅ merged |
| **P5.3** | **Spaces management nav** — Community "Spaces" + Admin "All Spaces" (rail is DB-menu-driven; live menu patched) | ADR-477 | live menu data patch | ✅ merged + patched |
| **P5.4** | **Space content-blocks + Spotlight tie-in** — extend the Spotlight block engine to Spaces; add Cover/Updates/Reviews/FAQ blocks; per-profile modular layout; small tables | [SPACE-MODE-TEMPLATES](SPACE-MODE-TEMPLATES.md) §5 | `space_updates`/`space_reviews`/`space_faqs` + `cover_image_url` (pending) | ⏳ awaiting architecture decision |
| **P5.5** | **Pricing surfaces rework** — public pricing page as the four-tier table, loadout picker tier chooser, admin pricing console UX, curated Modes per tier (TODO(ADR-472 surfaces)) | PRICING-LADDER-PLAN §1b · ADR-472 | none | ⏳ queued |
