...
>   was seeded here too; it left the code on 2026-08-24 with the Supporter tier (ADR-1106). HYG-078 /
>   ADR-1442 stops the seed and deletes the row on greenfield replay. Production: `execute_sql`
>   `delete from public.platform_flags where key = 'gamification_full_supporter'` (no ledger row).
...
| `web_role` | `text` (`none`/`admin`/`janitor`/`moderator`) | The **operational axis** (Site Admin / Executive Admin / Platform moderator), added in migration `20260613000050`, widened for `moderator` in `20270345006500` (OWN-054). Staff (`admin`/`janitor`) is the coarse gate for admin surfaces + janitor-only crown jewels; `moderator` is granted and is not staff. Read by `get_my_web_role()`. Orthogonal to `community_role` and to `membership_tier` (billing). The fine-grained per-domain staff layer is `team_members` (ADR-127). See [NAMING.md](NAMING.md) §Roles. |
...
| `web_role` | `none`, `admin`, `janitor`, `moderator` (`text` + CHECK, not a PG enum; migration `20260613000050`, widened `20270345006500`) |
