...
| A1 focal point | `lib/events/cover-focus.ts` | ✅ | The host's chosen crop for that cover, stored on the existing `events.theme` jsonb under `coverFocus` as a CSS `object-position` (no new column; sibling of `heroHeight`). `readEventCoverFocus(theme)` resolves it in a loader; `eventCoverFocusStyle(focus)` is THE render seam every cropped event cover pairs with `object-cover`. Unset resolves to the centered default, so a plain event crops exactly as before. **Every** cover surface uses it, not just the hero: the Catalog / marketplace / Space-block `EventCard`, the calendar + Space event popups, and both OG cards. A poster whose title sits at the top therefore survives the card's 16:9 crop the same way it survives the hero band. |
...
| Cards | `EventCard` (wraps `EntityCard`) | ✅ | The page's local `EventCard` already composes `EntityCard` with date block, warm badge, going count, host. **Add `cover` for events with a cover image** (EntityCard supports a `cover` 16:9 slot) so the Catalog reads like Luma Discover. The cover carries the host's focal point (`coverFocus` prop, from the index loader's `coverFocus` map) through `eventCoverFocusStyle`, so a card crops the photo identically to the page it links to (see §2.2 "A1 focal point"). |
