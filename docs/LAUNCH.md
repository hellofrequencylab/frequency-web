...
| `STRIPE_CONNECT_WEBHOOK_SECRET` | signing secret of a SECOND Stripe destination at the same URL, scoped **Events from: Connected accounts** (LIVE-215) | **optional until a host is onboarded**: a connected host's `account.updated` fires on the connected account, and only a Connect-scoped destination delivers it. Each destination has its own `whsec_`; the route tries `STRIPE_WEBHOOK_SECRET` first, then this one. Without it, a host whose payouts Stripe later disables keeps `stripe_payouts_enabled = true` forever |
| `STRIPE_PLATFORM_FEE_PCT` | flat platform fee % floor for Connect payout channels that have **no** space or member seller ladder | **optional**: defaults to **`3`**. Centralized in `lib/billing/fees.ts`; explicit `0` = no fee. 🔴 **Does NOT apply to tips: tips carry no platform fee on any tier (ADR-913).** Seller channels use the take-rate ladder instead (network-sourced only: Crew 8% · Business 5% · Non Profit 0%; own audience 0%) |
