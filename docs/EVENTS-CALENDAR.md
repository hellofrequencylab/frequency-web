...
**Admin / Guest on the Calendar tab** ([ADR-1389](DECISIONS.md), [ADR-1450](DECISIONS.md), [ADR-1456](DECISIONS.md)). A viewer who
...
shared with the settings console). Production is its own lane (`productionLane`). The board lists what is
penciled, in planning, and cancelled. `StaffCalendar` is the date map and the settings drawer, not a second guest month.
