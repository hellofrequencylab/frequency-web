...
**Admin / Guest on the Calendar tab** ([ADR-1389](DECISIONS.md), [ADR-1450](DECISIONS.md), [ADR-1454](DECISIONS.md), [ADR-1460](DECISIONS.md)). A viewer who
...
shared with the settings console). Pencil (`pencilLane`) and Planning (`planningLane`) are their own
lanes. The board lists what is in production and cancelled. `StaffCalendar` is the date map and the settings drawer, not a second guest month.
