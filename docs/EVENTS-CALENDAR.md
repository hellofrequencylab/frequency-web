...
**One header, and a month that fits** (PROG-CAL13, owner ask 2026-09-23; corrected by LIVE-475). The console's own header owns the month label and the Prev / Today / Next cluster, so the grid inside it draws neither: `EventCalendar` takes `hostChrome`, which drops the month title and the paging cluster and keeps everything else, including the `Loading` live region and the layer filters. It KEEPS two controls the host does not draw: the grid's own grid / list switcher (`aria-label="Calendar view"`, the month grid versus the same calendar as a chronological list — not the workspace's four-panel toggle, which is what the console header's view controls are) and the month-and-year jump, now a "Jump to a month" control on a slim strip since the month title it used to hang on is gone. Taking those two off with the rest of the header shipped a reachable dead end: switch the page grid to List, press Fullscreen, and you were in list mode inside a full-screen console with no way back to the month and no way to move more than one month at a time, with closing the console the only exit. `event-calendar.fill.render.test.tsx` and `calendar-workspace.render.test.tsx` pin the exit. On the page, where nothing else draws them, `hostChrome` is off and the grid's own header is unchanged. The console is a fixed-height panel and its tracks say so: header, side bar and stage are grid tracks, the stage row is `minmax(0,1fr)`, and `fill` makes the grid size to that row rather than to its content — the six week rows share it (`flex-1`) and a cell carries no minimum of its own, so all six weeks are on screen at any height and the panel never grows a scrollbar to reach the last one. A day holding more than its share scrolls INSIDE its cell (`overscroll-contain`), which is why `verticalScrollTaker` exists: a wheel or an Up / Down aimed at something that can still scroll keeps it, and the month only moves when nothing under the pointer or the key wants it. The side bar is the agenda over Ask Vera: the agenda takes the height and scrolls, Vera is a footer that stays put, so a long month never pushes it off screen and a proposal opens against a column that is still there.
...
panels on the page, and at the foot of the side bar inside the Calendar console, for the team that can edit the calendar (`components/spaces/vera-calendar-box.tsx`). A person picks
