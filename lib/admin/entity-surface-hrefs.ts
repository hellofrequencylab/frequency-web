...
export function hrefForEntitySurface(appId: string, scope: EntitySurfaceScope | null): string | null {
  ...
  if (entitySlug) {
    ...
    // circle.settings / event.settings render INLINE today (their module IS the editor). Their
    // standalone /settings pages retired under LIVE-237 (one editor per entity): the module now has
    // exactly one page home, the Manage hub's Settings tab. Listing it here (the account.profile
    // precedent above) means a `link` flip or a `placement: 'bank'` tag lands on that tab rather
