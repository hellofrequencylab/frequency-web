...
/** Day-of-month with an English ordinal suffix: 21 → "21st", 22 → "22nd", 23 → "23rd". */
 ...
 *  the target timestamp (for a live countdown), and a formatted start label ("June 21st").
 *  Impure (reads the clock), so it lives here in a plain helper, not in the view's render. */
export function seasonStartState(
  season: Season | null,
): { notStarted: boolean; startMs: number | null; startLabel: string | null } {
  const ms = season?.starts_at ? new Date(season.starts_at).getTime() : null
  if (ms == null) return { notStarted: false, startMs: null, startLabel: null }
  const d = new Date(ms)
  const month = d.toLocaleDateString('en-US', { month: 'long', timeZone: 'UTC' })
  return {
    notStarted: ms > Date.now(),
    startMs: ms,
    startLabel: `${month} ${ordinal(d.getUTCDate())}`,
  }
}
