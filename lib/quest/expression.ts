...
async function grantGemsOnce(
  ...
  if (error) {
    ...
    console.error('[expression.grantGemsOnce] reward_grants claim failed (not paid, retryable)', { ruleKey: ruleKey, error: error.message })
    return false
  }
    ...
    if (!wasDone) {
      if (opts.mode === 'circle') {
        ...
        if (!claimErr) {
          ...
          console.error('[completeExpressionChallenge] reward_grants claim failed (not paid, retryable)', { ruleKey: circleKey, error: claimErr.message })
        }
      } else {
