...
export async function getVisibleSpaceBySlug(
  ...
  // NOTE: this module is (transitively) reachable from client components, so it must NOT import lib/auth
  // (which pulls next/headers) — the platform-staff "view any Space" override lives in the SERVER-ONLY
  // gate wrapper (getVisibleSpaceForViewer) instead, never here.
