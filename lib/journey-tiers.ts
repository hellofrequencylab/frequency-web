...
// Spark / Current / Deep — resolves most-specific-first. Pure + framework-independent, so
...
export type IntensityTier = 'spark' | 'current' | 'deep'

export const INTENSITY_TIERS: readonly IntensityTier[] = ['spark', 'current', 'deep'] as const

/** The default when nothing else is set — the standard form. */
export const DEFAULT_TIER: IntensityTier = 'current'

/**
 * Resolve the tier a member sees for a step, most-specific-first:
 *   member override → circle default → item default → 'current'.
...
export function isIntensityTier(value: unknown): value is IntensityTier {
  return value === 'spark' || value === 'current' || value === 'deep'
}
