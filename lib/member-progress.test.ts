...
describe('deriveStage', () => {
  ...
  it('reaches established via 30-day streak, one Journey finished, or two adopted Journeys', () => {
    expect(deriveStage({ ...base, streak: 30 })).toBe('established')
    expect(deriveStage({ ...base, journeysFinished: 1 })).toBe('established')
    expect(deriveStage({ ...base, journeys: 2 })).toBe('established')
  })

  it('reaches anchor via a 100-day streak or two Journeys finished', () => {
    expect(deriveStage({ ...base, streak: 100 })).toBe('anchor')
    expect(deriveStage({ ...base, journeysFinished: 2 })).toBe('anchor')
  })
...
describe('gatesFor', () => {
  it('marks a gate met once its signal crosses', () => {
    ...
    expect(gates.find((g) => g.label.includes('Adopt'))?.met).toBe(false)
  })
