...
/** THE NARRATIVE SPINE, RE-EXPORTED. The constant itself moved to `./plan-story`, a LEAF module with
 *  no imports of its own (ADR-1368), so a caller that wants one approved sentence no longer has to
 *  take this module's nine-import runtime chain with it. That is not a style preference: it is what
 *  ADR-1363 refused to pay when it declined to derive a page-editor block default from
 *  `PLAN_STORY.paid`, because the fan-out gates that would have priced the import run only in
 *  `postbuild` on Vercel and the cost could not be measured before merging.
 *
 *  This re-export is the compatibility seam, so no existing importer changed. NEW callers outside
 *  `lib/pricing` should import from `@/lib/pricing/plan-story` directly and keep the copy free of the
 *  engine; importing it from HERE re-introduces exactly the chain the split exists to avoid. */
export { PLAN_STORY }
