...
//      Admin" / "Executive Admin") plus a granted Platform moderator (OWN-054).
//      NOT a ladder you climb — staff is who may enter admin surfaces
//      (admin/janitor) and the janitor-only crown jewels. Moderator is NOT staff:
//      it is a curated grant for platform-wide feed / library / achievement
//      moderation. It lives on `profiles.web_role` (migration 20260613000050,
//      widened 20270345006500) and is read via the WebRole helpers below. The
//      `team_members` staff matrix (lib/core/staff-roles) stays SIDE BY SIDE as
//      the fine-grained per-domain layer.
//
...
// the financials carve-out); `moderator` = curated Platform moderator (OWN-054),
// granted, never a side effect of publishing a Circle. Mirrors `profiles.web_role`'s
// CHECK constraint.

export type WebRole = 'none' | 'admin' | 'janitor' | 'moderator'
...
export function asWebRole(v: string | null | undefined): WebRole {
  return v === 'admin' || v === 'janitor' || v === 'moderator' ? v : 'none'
}

/** Platform STAFF = Site Admin or Executive Admin. The coarse "may enter admin
 *  surfaces / manage content I don't own" gate. A curated moderator is NOT staff. */
export function isStaff(webRole: WebRole | null | undefined): boolean {
  return webRole === 'admin' || webRole === 'janitor'
}

/** Curated Platform moderator (OWN-054). Granted. Never implied by community host. */
export function isCuratedModerator(webRole: WebRole | null | undefined): boolean {
  return webRole === 'moderator'
}

/** Staff or a granted Platform moderator. Feed, library review, and achievement
 *  grants use this. Admin chrome still reads isStaff. */
export function canModeratePlatform(webRole: WebRole | null | undefined): boolean {
  return isStaff(webRole) || isCuratedModerator(webRole)
}
