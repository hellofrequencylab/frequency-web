...
//
// The leadership walks MIRROR the broadcast association guard (app/(main)/broadcast/actions.ts):
// a circle is led by its host, its hub's guide, or its nexus's mentor; a hub by its guide or its
// nexus's mentor; a nexus by its mentor; an event by its host or an ACCEPTED cohost. Platform staff
// (web_role admin/janitor, ADR-208) pass every gate — they already hold the full cross-lane lens.
//
...
async function leadsNexus(senderProfileId: string, nexusId: string): Promise<boolean> {
  try {
    const admin = createAdminClient()
    const { data: n } = await admin.from('nexus_regions').select('mentor_id').eq('id', nexusId).maybeSingle()
    return n?.mentor_id === senderProfileId
  } catch {
    return false
  }
...
/**
 * May the sender DM this event attendee? Sender must LEAD the event (events.host_id, an ACCEPTED
 * cohost, or platform staff); target must actually be an attendee (event_rsvps status going|maybe —
 * the Message Attendees audience, owner ruling). Fail-closed on any error.
 */
export async function canDmEventAttendee(
  ...
  try {
    ...
    // The leadership leg: host, accepted cohost, or staff.
...
/**
 * May the sender DM this circle member? Sender must LEAD the circle — its host, the guide of its
 * hub, the mentor of the nexus above (the broadcast leadership walk), or platform staff; target must
 * hold an ACTIVE membership in the circle. Fail-closed on any error.
 */
...
/**
 * May the sender DM this hub/nexus subtree member? Sender must LEAD the scope — the hub's guide (or
 * the mentor above it) / the nexus's mentor — or be platform staff; target must hold an ACTIVE
 * membership in a circle of that subtree. The membership check is TARGETED (circle ids of the
 * subtree + one membership read filtered to the target), never a full 20k-id roster resolve.
  ...
  try {
    ...
    const leads =
      (scope.kind === 'hub'
        ...
        : await leadsNexus(senderProfileId, scope.id)) || (await isStaffProfile(senderProfileId))
    if (!leads) return false
