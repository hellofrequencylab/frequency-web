...
/** The optional, backward-compatible superset input for spaceSchema (P1:1, BUSINESS-ACCOUNTS-STRATEGY
 *  Area A). Every field beyond the base four is OPTIONAL and emitted only when present, so existing
 *  callers are unchanged. This is the single place the LocalBusiness/Organization node is shaped;
 *  later phases only PASS values (P1 SEO columns, P3 reviews -> aggregateRating, P6 sameAs/canonical),
 *  never re-declare the signature. */
export interface SpaceSchemaInput {
  ...
  /** A place or region served (for practitioners/services without a storefront). */
  areaServed?: string | null
  /** Emitted ONLY when reviewCount > 0. Never emit a null/zero rating: answer engines silently drop
   *  malformed schema, which would negate the SEO investment (reconciliation top risk). */
...
/** An ItemList of Space profiles, for a directory / programmatic hub page (P1 SEO). Each item points
 *  at the Space's public profile so an answer engine can walk the list to the entity nodes. */
