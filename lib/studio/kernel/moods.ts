// ─────────────────────────────────────────────────────────────────────────────
// THE STUDIO KERNEL — moods (docs/STUDIO.md, ADR-986).
//
// ONE dial that steers a whole generated draft: the TONE fed to the voice primer, the CTA
// POSTURE, and the design ACCENT emphasis. Born in the Business Seeder (Importer v2, owner
// request 2026-07-07) and promoted here unchanged, because a mood is not a business concept:
// a Journey, an Event, or a Circle drafted "calm and trustworthy" should read that way too.
//
// Add a mood here and EVERY entity's Spark and re-seed offers it. That is the point.
//
// PURE metadata: no IO, no AI, no React, and (deliberately) no entity types. The mapping from
// a mood to a SPACE PAGE THEME is a Space concern and stays with the Space entity, not here.
//
// Voice canon (docs/CONTENT-VOICE.md): operator-facing copy here stays plain, no em dashes.
// ─────────────────────────────────────────────────────────────────────────────

/** The four moods (owner pick, 2026-07-07). */
...
/** Whether a value is a known mood key. */
export function isSeedMood(value: unknown): value is SeedMood {
  return typeof value === 'string' && BY_KEY.has(value as SeedMood)
}

/** Coerce any value to a valid mood, falling back to the default. Total + safe for stored input. */
...
/** The tone line a mood contributes to the voice primer. Empty-safe. */
